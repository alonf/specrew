...
Show the full Specrew slash-command catalog and provide next-step guidance. This is the **canonical fallback catalog** for every supported environment — reliable even when host-native `/specrew.` prefix discovery is unavailable or incomplete.
...
- As a discovery fallback in any environment where `/specrew.` prefix suggestions are degraded or unknown.
...
Part of the `/specrew.*` namespace. The Specrew catalog help must never absorb or shadow `/speckit.*` lifecycle commands. Both namespaces remain independently discoverable.
