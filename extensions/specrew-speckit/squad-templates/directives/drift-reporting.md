...
After every task, check the delivered output against its source requirement and record drift before the next task begins.
...
1. **Check for drift after every task**
   - When a task is ready to move to `done`, invoke `specrew-drift-check`
