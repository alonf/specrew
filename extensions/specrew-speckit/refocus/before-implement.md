...
5. **After the verdict, run — and not one source file before it.** Until the boundary ledger holds the human's typed `approved for before-implement`, no product source file is created or modified; no other approval substitutes, and a go-ahead you inferred is not one the ledger recorded. Once that verdict IS recorded, proceed through implement → review → retro without re-asking per phase — but every gate still preflights and every boundary still commits.
