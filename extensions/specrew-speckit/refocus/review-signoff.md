...
4. **Live co-review is mandatory for code-touched review-signoff.** Before accepting `review.md`, invoke `/specrew-review --live` (or `specrew review --live`) with the active feature and iteration context. Treat `.specrew/review/inline/<run-id>/findings-result.json` / `review-run.json` (the worktree co-review's durable evidence) as REQUIRED — `review.md: accepted` is invalid without it. A NON-ZERO `specrew review --live` (e.g. `no-authorized-reviewer-host`) is an INFRASTRUCTURE BLOCKER, not a clean review: surface its remediation (`specrew review --host <host> --authorization-ref <ref>`) and record a blocker or obtain explicit human defer approval. Do NOT replace the co-review with ANY substitute — not hand-authored prose, and NOT a host-internal code-review agent or your own manual read. Those may find real bugs, but they are NOT co-review evidence and must NEVER be recorded as the iteration's review verdict. (A host filling the co-review vacuum with its own reviewer, then writing `review.md: accepted`, is the exact failure this guards.)
