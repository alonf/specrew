...
9. **At ANY stop where you yield the turn to the human for a decision — every boundary gate AND every within-phase task checkpoint or "proceed?" hand-off, not only formal lifecycle boundaries — render the full six-section re-entry packet** as a message (What I Just Did / Why I Stopped / What Needs Your Review / What Happens Next / Discussion Prompts / What I Need From You), never collapsed into a picker/menu, which hides the packet behind its short fields. At a BOUNDARY stop, ALSO emit the verdict marker `<!-- SPECREW-VERDICT-BOUNDARY: <from> -> <to> -->` as the LAST line so the human's verdict is captured into the gate's authorization. Render and collect the verdict through your host's approved interaction path for a verdict stop: use your host's dedicated boundary-stop surface if it has one, otherwise render the packet directly — full sectioned markdown with bare `file:///` URLs and a numbered verdict block, never a compressed "1. Why:… 2. What:…" list. The design workshop's per-lens questions and clarify-stage ambiguity questions are NOT decision-yield stops — they keep their normal picker/question path.
