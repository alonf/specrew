...
6. **Stop at the boundary.** Commit `boundary(specify): write spec.md`, sync state, read `.specrew/runtime/pending-verdict-stop.md` if it exists, and render the verdict packet through your host's approved interaction path for a verdict stop (its dedicated boundary-stop surface if it has one, otherwise render the packet directly) — as a full message, never collapsed into a picker/menu — then emit the artifact's exact marker as the LAST line. For the first specify gate that marker is `<!-- SPECREW-VERDICT-BOUNDARY: intake -> specify -->`, where `intake` is the marker-only name for the pre-specify side of the first gate; wait for the human verdict before clarify work.
