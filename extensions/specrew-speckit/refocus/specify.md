...
6. **Stop at the boundary.** Commit `boundary(specify): write spec.md`, sync state, and render the verdict packet. On Claude, invoke `specrew-gate-stop` (picker disabled, packet rendered as prose); on non-Claude hosts, render directly through that host's approved path. Wait for the human verdict before clarify work.
