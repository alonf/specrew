...
6. **Stop at the boundary.** Commit `boundary(specify): write spec.md`, sync state, emit the re-entry packet, and wait for the human verdict before clarify work.

Known traps: menu-before-render (the AskUserQuestion tool-gravity); fabricated lens agreement; a spec that transcribes the request instead of the decisions; missing checklists/requirements.md.
