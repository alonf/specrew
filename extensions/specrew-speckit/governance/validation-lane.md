...
The handoff-governance slice is complete when the authorized validator command and both integration tests pass from the repository root.

1. Run the validator directly against representative coordinator output when evaluating a final response.
2. Run both integration tests in the validation lane before iteration closeout.
