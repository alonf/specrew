...
for ($i = 0; $i -lt $args.Count; $i++) {
    ...
    # --host-kind / --transcript-path are accepted-but-unused by the navigator (it works off git state +
    # the registry, not the transcript); consuming them here keeps the manual parser tolerant.
}
