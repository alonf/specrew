...
After `/speckit.tasks` updates `tasks.md`, record the boundary arrival FIRST — the sync mints the
pending crossing and writes `.specrew/runtime/pending-verdict-stop.md`, the controller truth the
verdict stop renders from (DRIFT-198-I011-012: gating before the sync left the stop with no
artifact, the marker-invention precondition):
