---
description: "Persist session-state metadata after retro boundary work is committed"
---

# Sync Retro Boundary State

## Record the arrival (before the advancement gate)

After retro artifacts are committed to the feature branch, record the boundary arrival FIRST — the
sync mints the pending crossing and writes `.specrew/runtime/pending-verdict-stop.md`, the
controller truth the verdict stop renders from (DRIFT-198-I011-012: gating before the sync left the
...
pwsh -File .\.specify\extensions\specrew-speckit\scripts\sync-boundary-state.ps1 -ProjectPath . -BoundaryType retro -FeatureRef $featureRef -IterationNumber $iterationNumber
```
...
$authorization = Test-SpecrewBoundaryAuthorization -ProjectRoot . -CurrentBoundary 'review-signoff' -RequestedBoundary 'retro'
...
This command replaces inline PowerShell invocation of sync-boundary-state.ps1 for the retro boundary. Use this command (not inline PowerShell, and not manual state-file edits) so the canonical sync logic fires correctly and state files end up in the canonical post-boundary state.
