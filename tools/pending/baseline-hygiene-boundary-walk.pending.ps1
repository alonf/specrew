# RELOCATED OUT OF tests/ 2026-09-04 - THE BOUNDARY WALK ONLY. The rest of baseline-hygiene stays in the
# suite and still runs: 13 check sites covering the baseline helper - frontmatter and body preserved
# across a hash refresh, Get-BaselineCommitHash returning null for missing/invalid/malformed prompts,
# graceful rehydration, fail-closed on write errors. Only this block is deferred.
#
# WHAT IT WAITS ON: a fixture that DRIVES THE REAL LIFECYCLE instead of staging artifacts around it.
#
# WHY. This walk crosses all nine boundaries against a hand-staged project, and it now has to satisfy
# four gates that were each added independently to enforce an ordering the product produces naturally:
#   1. the owed-artifact MINT GUARD (FR-024/T014) - the entered stage's artifacts must exist on disk
#      BEFORE the crossing into it can open;
#   2. the STATE-TRUTH gate - state.md must MIRROR the last authorized boundary, lagging the authority
#      record and never leading it;
#   3. the HONEST-STATE FLOOR - state.md must exist, and must not read as a scaffold, once evidence
#      artifacts like review.md exist;
#   4. the GENUINE-CHANGE path - no iteration evidence may exist before an iteration does.
#
# Staging artifacts satisfies three and breaks the fourth, in rotation. Measured: successive fixes moved
# the failure clarify -> before-implement -> tasks -> review-signoff -> before-implement -> clarify ->
# tasks, each one a real advance and none converging.
#
# THE CONSTRAINTS ARE DEMONSTRABLY SATISFIABLE - every real project crosses these boundaries daily. What
# is not satisfiable is FAKING the states the product creates as a side effect of its own operations.
# So the fix is not a rewrite in the vague sense: drive the real lifecycle and let each boundary produce
# its own artifacts, exactly as docs/beta3-candidate-walk.md already does for the release walk. The
# technique exists; this fixture predates it.
#
# STALENESS CITATION: FR-024 / T014, iteration 002 - the owed-artifact mint guard broke it first and is
# deliberate batch work.
#
# THE GATE DOES NOT RUN THIS. Run it by hand while working the replacement:
#   pwsh -NoProfile -File tools/pending/baseline-hygiene-boundary-walk.pending.ps1
