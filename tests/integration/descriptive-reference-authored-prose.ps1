...
$fixturesRoot = Join-Path $repoRoot 'tests\integration\fixtures\descriptive-reference-authored-prose'
...
$fixtureDirectories = @(
    Get-ChildItem -LiteralPath $fixturesRoot -Directory -Recurse |
        Where-Object { Test-Path -LiteralPath (Join-Path $_.FullName 'fixture.psd1') -PathType Leaf } |
        Sort-Object FullName
)

if ($fixtureDirectories.Count -eq 0) {
    Write-Fail "No authored-prose fixtures found under $fixturesRoot"
    exit 1
}
...
foreach ($fixtureDirectory in $fixtureDirectories) {
    ...
    Write-Pass ("Fixture '{0}' replayed through the real governance validator path with the expected user-visible output." -f $manifest.FixtureId)
}

Write-Pass 'Authored-prose replay fixtures exercise the real governance review path and preserve soft-warning behavior'
exit 0
