...
| `error-handling-expectations` | `error-handling` | `addressed` | `planning-time-analysis` | `pending-post-implementation` | Structured FAIL output remains visible and additive. | `false` | Fixture keeps graceful error handling explicit before runtime proof exists. | — |
...
| `fixture-follow-through` | `test-integrity` | `addressed` | `planning-time-analysis` | `pending-post-implementation` | Extra feature-specific concern rows may appear after the canonical five. | `false` | This row proves additional concerns are allowed after the first five canonical rows. | — |
