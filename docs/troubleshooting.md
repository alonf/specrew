...
After `specrew init`, you normally just launch your host — run `claude`, `codex`, `copilot`, or `cursor` — and Specrew greets you with an orientation banner and drives the governed lifecycle. Antigravity has bounded `.agents/hooks.json` support for `PreInvocation` bootstrap injection and `Stop` handover decisions; if that host does not fire those hooks, start or recover with `specrew start --host antigravity`.
