...
- **Hook triggers (per host, per the current matrix)**: Claude and Codex use `UserPromptSubmit`, Copilot CLI uses `userPromptSubmitted`, Cursor Agent CLI uses `beforeSubmitPrompt`, and Antigravity uses `PreInvocation` as their genuine per-prompt boundary. Those events refresh owner-scoped conformance turn baselines and carry B3 checks. Session-start/equivalent hooks still bootstrap and re-ground each host; Antigravity's project-local `.agents/hooks.json` also uses `Stop` for handover and has no B1 compaction carrier. Hooks deploy to per-user config where the host supports it; Antigravity's hook config is project-local and merge-aware. All hosts respect a recorded opt-out.
...
| Hooks (PreToolUse, etc.) | ✅ Session-start + `userPromptSubmitted` | ✅ Session-start + `UserPromptSubmit` | ✅ Session-start + `beforeSubmitPrompt` | ✅ Full refocus triad with `UserPromptSubmit` | `PreInvocation` bootstrap + per-prompt B3/turn baseline, `Stop` handover; no B1 compaction carrier; use `specrew start --host antigravity` if hooks do not fire |
