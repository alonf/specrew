...
## Session Continuity — Hook Bootstrap, Rolling Handover, Host Switching (Feature 174)

Sessions end — `/exit`, a crash, a closed window, or simply switching to a different AI host. Specrew treats every restart as a resume problem and solves it with two hooks plus a deterministic disk scan, so the next session starts where the last one stopped instead of asking "what do you want to build?".

- **SessionStart bootstrap (the way in)**: launching a governed host (`claude` / `codex` / `copilot` / `cursor`) inside the project fires a SessionStart hook that classifies the situation — `welcome-back` when a valid session anchor or a validated handover exists, `full` otherwise — and injects the orientation banner, the governed launch contract, and the resume context as the agent's first-turn input. Every classification is journaled to `.specrew/runtime/bootstrap-journal.jsonl` (one JSON line per session start: mode, handover validity, concurrency signals) — the first place to look when a resume behaves unexpectedly.
- **Stop-hook rolling handover (the way out)**: a companion Stop hook maintains ONE crash-safe file, `.specrew/handover/session-handover.md`, overwritten in place on each material turn-end. Its frontmatter floor (feature, boundary, commit, timestamp) is written mechanically — the feature is resolved from the session anchor or, during the pre-specify workshop window, from the git branch. Its seven body sections (five hook-authored mechanical sections — including the best-effort `Recent conversation` tail — plus two interpretive sections only the agent can author) carry richer context; the hook preserves an authored body and writes an explicit placeholder otherwise — a `HOLLOW HANDOVER` warning on resume means the body was never authored, and the agent re-derives from artifacts instead. The file is written atomically (a per-PID temp swapped into place, keeping a `.old` backup the reader falls back to), so a kill landing mid-write never corrupts it.
- **In-flight work surfacing**: on every session start the bootstrap also scans disk truth — `specs/<feature>/spec.md` (the intent), the workshop records and `lens-applicability.json` (lenses done vs remaining) — and tells the agent concretely where to resume ("resume the design workshop at the next remaining lens: component-design"; "the workshop is COMPLETE — resume at the boundary verdict"). This works even when no handover exists at all, so a hard kill loses position but never the work.
- **Host switching (Proposal 130)**: the handover is host-portable by design. Exit one host, launch a *different* one in the same project, type `continue` — the new host validates the previous host's handover, classifies `welcome-back`, and picks up the same feature at the same spot. Verified live with a three-way rotation (Claude resuming a Copilot session, Codex resuming Claude's, Copilot resuming Codex's).
- **Honest limits — what a crash costs, and what always survives**: durable state — every committed artifact and the working tree — ALWAYS survives a crash and is re-derived by the next session's disk scan; only the *non-durable conversation tail since the last capture* can be lost. A hard kill (SIGKILL, power loss, a force-closed window) fires NO stop event on ANY host, so it writes no handover floor for that session; the resume then rides the previous floor plus the disk scan. How much conversation is at risk depends on the host's capture cadence: on Claude the handover refreshes every PostToolUse (seconds of exposure), so a crash loses little; on Codex / Copilot / Cursor the capture is the last *graceful* Stop, so an ungraceful kill loses back to there. This conversation-loss floor is UNIVERSAL and uncloseable — no hook can fire after the process is already gone.
- **Antigravity — recovers, but cannot capture**: Antigravity has no hook surface at all, so it can neither fire a SessionStart bootstrap nor write a rolling handover — it never records the conversation. It RECOVERS instead through `specrew start`, which reads the handover written by whichever host last ran and executes the SAME reconciliation, and its own work always survives on disk for the next session to re-derive. So an Antigravity session is fully governed and resumable; it just contributes no conversation capture of its own. And boundary discipline is unchanged by all of this: a resuming agent still may not advance a human-judgment gate without your verdict.
