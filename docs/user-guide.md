...
`specrew start` is the canonical downstream entrypoint. It prepares the Squad handoff, launches Copilot CLI when available, and tells Squad to drive the full Spec Kit lifecycle (`specify`, `clarify` when needed, `plan`, `tasks`, `implement`). The intended human role is to answer only the unresolved questions Squad cannot safely answer from repo context or current artifacts. You can optionally pass a short plain-language request if you already know the next feature or fix. To reduce Copilot CLI blocking, Specrew launches from the project directory and defaults to `--allow-all`; use `--prompt-approvals` if you want Copilot to keep interactive permission prompts. Copilot may still ask you to trust the project directory on first launch.
