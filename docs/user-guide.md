...
Once the scope is grounded, Specrew launches from the project directory, reuses the current terminal by default, and auto-loads a compact bootstrap message via `-i` that points Copilot at `.specrew\last-start-prompt.md`, `.specrew\start-context.json`, and the human-readable `.specrew\start-summary.md`. Windows keeps `--allow-all` enabled by default. Linux/macOS intentionally suppress `--allow-all` for now because Copilot CLI v1.0.48 does not reliably preserve the intended interactive behavior there, so expect approval prompts for bootstrap file reads until a later Copilot CLI release fixes that interaction. When autopilot is off, Specrew also passes `--mode interactive` explicitly so the REPL stays open. Use `--prompt-approvals` for interactive permission prompts or `--new-window` if you explicitly want a detached PowerShell window.
