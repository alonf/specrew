...
Once the scope is grounded, Specrew launches from the project directory, defaults to `--allow-all`, reuses the current terminal by default, and auto-loads a compact bootstrap message via `-i` that points Copilot at `.specrew\last-start-prompt.md`, `.specrew\start-context.json`, and the human-readable `.specrew\start-summary.md`. When autopilot is off, Specrew also passes `--mode interactive` explicitly so the REPL stays open. Use `--prompt-approvals` for interactive permission prompts or `--new-window` if you explicitly want a detached PowerShell window.
