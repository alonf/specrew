...
- **Honest limits**: a hard kill (or a host whose stop event only fires at full turn-completion, like Copilot mid-question) writes no floor for that session — the resume then rides the previous floor plus the disk scan. And boundary discipline is unchanged by all of this: a resuming agent still may not advance a human-judgment gate without your verdict.
