...
Once the scope is grounded, Specrew launches from the project directory, defaults to `--allow-all`, reuses the current terminal by default, and gives Copilot a compact bootstrap message that points it at `.specrew\last-start-prompt.md`, `.specrew\start-context.json`, and the human-readable `.specrew\start-summary.md` instead of pasting the full handoff prompt into the terminal. Use `--prompt-approvals` for interactive permission prompts or `--new-window` if you explicitly want a detached PowerShell window.
