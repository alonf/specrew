...
Once the scope is grounded, Specrew launches from the project directory, defaults to allow-all/yolo approval semantics, reuses the current terminal by default, and prints a compact bootstrap block that points Copilot at `.specrew\last-start-prompt.md`, `.specrew\start-context.json`, and the human-readable `.specrew\start-summary.md`. Paste that block as your first Copilot message after the REPL opens. Use `--prompt-approvals` for interactive permission prompts or `--new-window` if you explicitly want a detached PowerShell window.
