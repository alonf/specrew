...
| **Antigravity** | `antigravity` | `agy` | [antigravity.google](https://antigravity.google/) | Native entry point is `agy` in the project; `specrew start --host antigravity` launches `agy -i <prompt> --add-dir <path>`. `agy -c` or `agy --conversation <id>` resumes a prior Antigravity conversation. Specrew hooks use project `.agents/hooks.json`: `PreInvocation` for bootstrap + B3 refocus and `Stop` for handover. `--dangerously-skip-permissions` maps from `--allow-all`. |
