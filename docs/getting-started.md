...
4. Hands the Crew the feature description and tells it to drive the canonical lifecycle. The nine **boundaries** — the points where work stops and waits for your typed verdict — are: `specify → clarify → plan → tasks → before-implement → review-signoff → retro → iteration-closeout → feature-closeout`. Implementation is a **stage, not a boundary**: the code is written between `before-implement` and `review-signoff` without stopping for a verdict in between, which is why there is no `approved for implement`
...
When the Crew surfaces a clarify question, answer it. When it surfaces a planning artifact, review it. When it asks for a verdict, type the phrase for **the boundary named in the packet** — `approved for plan`, `approved for tasks`, `approved for before-implement`, and so on, using the boundary names in the list above.

**Two rules about that phrase, because only a typed phrase is captured** — both cost real retries before they were written down:

1. **The phrase must be the first thing in your message.** It is matched from the start of what you type, so `ok, approved for plan` does not match and nothing is recorded.
2. **A rationale runs from the dash to the end of your message** — not to the end of the sentence — with a 2,000-character limit. So `approved for plan - the schema is right` is fine, but adding two more paragraphs of instructions below it makes the *whole remainder* the rationale, and it can be rejected for length. **Send instructions as a separate message.**

Full detail, including what a rejected phrase tells you: [docs/user-guide.md "Recognized verdict shapes"](user-guide.md#recognized-verdict-shapes).
...
**The Design Workshop — this is what happens first, before any spec.** Walked on three hosts in this line: after you state the feature, the very next thing is not a spec and not a clarify question, it is the workshop's **intake lens** (product & problem domain), and the spec is written *from* it. Expect to answer questions in prose and to be shown a lens agenda before anything is written down. For substantive features, the Crew facilitates the workshop — first at intake (it hands you a lens agenda, opens with the **product & problem domain** lens to ground users / pain / MVP / constraints, then works the technical lenses that apply) and again at the design-analysis stop before planning (to co-design the architecture with you: component map, responsibilities, flows, and trade-off options). For any code-writing feature it also runs the **code & implementation** lens, which captures implementation-craft rules (language version, patterns, per-stack conventions, dependency choices) into a per-feature `implementation-rules.yml` manifest, which the lens's implement-time guidance skill then uses to guide the coding agent while it writes code. It is a conversation, not a questionnaire — you see every diagram and agenda in-band, and every decision is recorded as a durable artifact. The full methodology is in [docs/methodology/design-workshop-methodology.md](methodology/design-workshop-methodology.md).
