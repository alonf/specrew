...
To reduce Copilot CLI blocking on tool prompts without collapsing the session into a one-shot run, Specrew launches Copilot from the target project directory, reuses the current terminal by default, keeps allow-all/yolo approval semantics unless you opt into `--prompt-approvals`, and prints a compact bootstrap block that points at `.specrew\last-start-prompt.md`, `.specrew\start-context.json`, and `.specrew\start-summary.md`. When the Copilot REPL opens, paste that block as your first message. Copilot may still ask you to trust the project directory on first launch. If you prefer Copilot's interactive approval prompts, use:
