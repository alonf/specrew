...
**Input**: F-049 user request: Docker pre-publish version-update validation, durable troubleshooting guide docs, persona-driven `/speckit.specify` intake, approved Proposal 120 five-pillar bypass-detection scope reserved for Iteration 004, and Proposal 141 **Crew Interaction Profile / Persona Lens Separation** reserved for Iteration 005.
...
### User Story 3 - Persona-Driven Substantive Specification Intake with Expertise-Aware Depth and Crew Interaction Profile / Persona Lens Separation (Priority: P3)

To ensure that the initial specify phase (`/speckit.specify`) captures realistic scope, technology constraints, and organizational context—while respecting each user's expertise level—it must utilize an **engine + data architecture** where a discrete intake engine orchestrates question flow, persona application, and auto-decision resolution, while all persona definitions, question banks, depth rules, and auto-decision defaults are persisted as YAML data outside engine code. The intake architecture continues to use 4 internal persona lenses, while the human-facing profile experience presents a **Crew Interaction Profile**: 4 decision-area settings that tell Specrew how much to ask, explain, recommend, and auto-decide without asking the user to identify as an internal job title.

**Why this priority**: Substantially reduces clarify-phase back-and-forth by ensuring that the spec is born stack-aware, persona-aligned, and expertise-adapted. Higher-profile values let Specrew ask concise expert-level questions and assume the human wants to make the call; lower values or `auto` tell Specrew to explain more, recommend defaults, and surface transparent auto-decisions (Proposal 053 pattern). All users set their profile once and reuse it across all Specrew projects. The engine + data separation enables future expansion (new personas, categories, domain bundles, stack-specific defaults) as **data-only additions** without code rewrites. Proposal 141 adds a bounded correction so the user profile is clearly framed as a **Crew Interaction Profile** while preserving the internal lens architecture, stable persisted keys, and Iteration 003 intake behavior.
...
**Crew Interaction Profile Values** (4 user-facing decision areas, 1-10 scale, persisted to `~/.specrew/user-profile.yml` using stable schema keys):
...
**Independent Test**: Execute the new `/speckit.specify` command under various user expertise levels and input states, verifying that (a) Crew Interaction Profile values control question depth per lens, (b) per-lens mode evaluation produces the most-conservative mode as overall intake depth, (c) auto-decisions surface with transparency, (d) user-profile.yml persists across invocations, (e) the resolved current-user profile is surfaced in session context as soft collaboration guidance, and (f) correct persona-driven spec layout is generated.

**Acceptance Scenarios**:

1. **Given** a user running `specrew start` for the first time (user-profile.yml absent), **When** the bootstrap completes, **Then** the system prompts for a **Crew Interaction Profile** self-rating across the 4 decision areas (**Product Strategy**, **UX/UI Design**, **Software Architecture**, **AI Delivery Planning**) using the existing persisted schema keys, saves the profile, and surfaces a summary that distinguishes the user's profile from Specrew's internal persona lenses.
2. **Given** a user with an existing user-profile.yml, **When** they run `specrew start`, **Then** the profile is read without migration and a session summary (with `/specrew-user-profile edit` or `/specrew-user-profile reset` guidance) is surfaced using Crew Interaction Profile wording rather than job-title identity wording.
3. **Given** a user with an existing user-profile.yml, **When** session context is prepared for agents, **Then** the resolved current-user profile is surfaced as soft collaboration guidance describing how much to ask, explain, recommend, and auto-decide, and it is marked as user-specific runtime context rather than shared project truth.
4. **Given** two developers working in the same repository with different local user-profile.yml files, **When** each starts a session, **Then** each developer receives their own resolved Crew Interaction Profile guidance without requiring shared repository changes.
5. **Given** a user initiating `/speckit.specify`, **When** the agent begins intake, **Then** it applies all 4 sequential internal persona lenses to the same input, covering all 12 categories from each perspective, while the user's saved Crew Interaction Profile values determine depth and auto-decision behavior for those lenses.
6. **Given** a chosen Crew Interaction Profile value (e.g., Software Architecture: 7), **When** intake questions are presented:
...
7. **Given** any question during intake, **When** the user is unsure, **Then** they can choose `"Other"` or `"I don't know, you decide"` to trigger proactive agent domain research or auto-derivation of stack-aware defaults.
8. **Given** intake mode branching (Mode A/B/C) is evaluated **per-lens**, **When** the system evaluates each persona lens against its own expertise dial and the completeness of that lens's content:
...
9. **Given** low-expertise auto-decisions (dial 1-3), **When** the spec is generated, **Then** each auto-decision is annotated with `[AUTO-DECIDED: <decision>]` so the user sees what the system chose and can escalate to clarification if needed.
10. **Given** a legacy `user-profile.yml` created before the Crew Interaction Profile wording correction, **When** it is loaded by `specrew start`, `/specrew-user-profile`, or `/speckit.specify`, **Then** the same persisted keys and internal persona IDs remain valid, the same depth behavior is preserved, and only the visible labels/explanatory text change.
...
- **Session Context User Specificity**: Session-local guidance may summarize the resolved current user's Crew Interaction Profile, but durable shared repository instructions must not present those resolved values as global project truth.
- **Multi-Developer Local Divergence**: Two developers in the same repository may have different local `user-profile.yml` values. Runtime guidance must vary per developer without requiring or producing shared repository diffs.
...
- **FR-023**: `/speckit.specify` MUST prompt the user to complete a Crew Interaction Profile self-rating on a 1-10 scale for each of the 4 user-facing decision areas (**Software Architecture**, **UX/UI Design**, **Product Strategy**, **AI Delivery Planning**), with an escape hatch option `"I'm new, you decide"` per area.
...
- **FR-026**: `specrew start` MUST detect first-run (user-profile.yml absent), prompt for Crew Interaction Profile self-rating before bootstrap completes, save the profile, and surface a profile summary in `start-context.json` and `start-summary.md`. On subsequent runs, `specrew start` MUST read the profile and surface the profile summary plus reset/edit guidance.
...
The `specrew start` first-run Crew Interaction Profile intake and the `/specrew-user-profile` command enable durable user profiling across all Specrew projects (user-level, not project-level). `/speckit.specify` consumes this profile to personalize question depth and auto-decision behavior per persona lens.
...
#### Iteration 5: Crew Interaction Profile / Persona Lens Separation

- **FR-032**: `specrew start`, `/specrew-user-profile`, `/speckit.specify`, and any first-run/profile summaries MUST describe the 4 user inputs as a **Crew Interaction Profile** for the decision areas **Product Strategy**, **UX/UI Design**, **Software Architecture**, and **AI Delivery Planning**, not as job titles, identity claims, or professional badges the user must personally hold.
...
- **FR-034**: The fourth visible Crew Interaction Profile decision area MUST be labeled **AI Delivery Planning** everywhere the profile is presented to the user. This visible label MUST map to the existing fourth internal lens (`AI Researcher / Project Manager`) and continue to cover capacity planning, safe parallelism, specialist pairing, agent charters, delivery risk, and AI-agent workflow design.
- **FR-035**: Profile summaries, intake guidance, and related help text MUST explicitly distinguish **the user's Crew Interaction Profile** from **Specrew's internal persona lenses**. The copy MUST explain that higher values permit concise expert-level questions and more user-led decisions, while lower values or `auto` require more explanation, stronger recommendations, and transparent auto-decisions.
- **FR-036**: Documentation, slash-command skills, and reviewer/operator guidance for substantive intake MUST explain the two-level behavior consistently: the Crew Interaction Profile is soft collaboration guidance for all agents in session context, `/speckit.specify` is the only hard-applied behavior in this release, Proposal 120 remains Iteration 004 unchanged, and role/gate-specific adaptation elsewhere remains future work.
- **FR-037**: Tests and scripted evidence MUST prove compatibility for pre-existing `user-profile.yml` files by showing that legacy profiles load without migration, preserve the same internal lens routing and `/speckit.specify` question-depth behavior, and surface the updated Crew Interaction Profile wording in user-facing output.
- **FR-038**: `specrew start` and related session-context generation MUST surface the resolved current user's Crew Interaction Profile in runtime session context as soft collaboration guidance for all agents, including how much to ask, explain, recommend, and auto-decide, while clearly marking it as user-specific runtime context rather than shared project truth.
- **FR-039**: Durable shared instructions and shared agent guidance MUST reference the current user's profile path and loader rule, not hard-coded dial values or a specific developer's resolved settings. The rule MUST point to `$env:USERPROFILE\.specrew\user-profile.yml` on Windows and `~/.specrew/user-profile.yml` on Unix-like systems, or to the shared loader that resolves those paths at session start.
- **FR-040**: Outside `/speckit.specify`, the Crew Interaction Profile MUST remain soft guidance only in this release. No other role, gate, or lifecycle surface may claim hard behavioral enforcement unless a later approved feature explicitly adds that enforcement.
- **FR-041**: Multi-developer safety MUST be preserved: different developers working in the same repository can have different local `user-profile.yml` files, receive different resolved session guidance, and avoid shared repository changes or committed per-developer profile values.
...
- **TG-003**: User Story 3 maps to FR-008, FR-009, FR-010, FR-011, FR-023, FR-024, FR-025, FR-026, FR-027, FR-028, FR-029, FR-030, FR-031, FR-032, FR-033, FR-034, FR-035, FR-036, FR-037, FR-038, FR-039, FR-040, FR-041, SC-003, SC-005, SC-006, SC-007, and SC-008.
...
- **TG-006**: F-049 is an approved **five-iteration** feature. Iterations 001-003 are closed; Iteration 004 remains the reserved Proposal 120 bypass-detection slice; Iteration 005 is the approved Proposal 141 **Crew Interaction Profile / Persona Lens Separation** slice.
- **TG-007**: Iteration delivery window:
  ...
  - **Iteration 003 (closed)**: FR-008 to FR-011, FR-023 to FR-031 (Persona-driven specify intake with expertise-aware Crew Interaction Profile values, user profile persistence, slash command, and **engine + data architecture foundation**).
  - **Iteration 004**: FR-018 to FR-022 (Proposal 120 full five-pillar bypass detection).
  - **Iteration 005**: FR-032 to FR-041 (Proposal 141 Crew Interaction Profile / Persona Lens Separation slice covering wording, soft session guidance, durable loader/path rules, hard `/speckit.specify` boundary, and multi-developer safety without schema or lens-architecture changes).
...
- **TG-009**: FR-023 (Crew Interaction Profile self-rating) is the intake foundation; FR-024 (user-profile.yml) enables persistence; FR-025 (`/specrew-user-profile`) enables user control.
- **TG-010**: FR-026 (specrew start first-run integration) surfaces Crew Interaction Profile intake at bootstrap time; FR-027 (`/speckit.specify` profile consumption) applies the profile to question depth and auto-decision behavior.
- **TG-011**: Crew Interaction Profile semantics follow the 7-10 / 4-6 / 1-3 / "I'm new" rules: higher values allow terser expert-level questioning and more user-led judgment, while lower values require more explanation, stronger recommendations, and transparent auto-decisions. Low-expertise auto-decisions MUST be surfaced via Proposal 053 transparency pattern, never silent.
- **TG-012**: User-profile.yml is user-level (persisted across all Specrew projects), NOT project-level; it is the user-level analogue to Proposal 047 project-governance profile. Shared instructions must load the **current user's** profile at session start rather than embedding resolved per-user values into repository files.
...
- **TG-017**: Proposal 141 is a bounded **Iteration 005 follow-on correction slice**, not a reopening of Iteration 003. Iteration 005 updates Crew Interaction Profile framing, soft session-context guidance, durable loader/path rules, multi-developer safety, and compatibility evidence only; it does not split persona architecture, add a fifth lens, or migrate persisted profile data.
- **TG-018**: Iteration 005 behavior is fixed as follows: user-facing Crew Interaction Profile labels are **Product Strategy**, **UX/UI Design**, **Software Architecture**, and **AI Delivery Planning**; internal lens execution remains **Product Manager**, **UX/UI Specialist**, **Architect**, and **AI Researcher / Project Manager**; persisted keys and persona IDs remain unchanged; `/speckit.specify` is the only hard-applied behavior in this release; and durable shared instructions point to the current-user profile loader/path rule rather than concrete dial values.
...
- **SC-003**: `/speckit.specify` with Crew Interaction Profile values generates highly contextual specs tailored to the 4 internal persona lenses, respecting user expertise levels (senior, standard, learning), with less than 2 subsequent clarify questions in 90% of runs when the relevant decision-area value is 4+.
...
- **SC-005**: Crew Interaction Profile values and user-profile.yml persistence deliver empirical evidence of question reduction and per-lens mode branching effectiveness:
...
- **SC-007**: Proposal 141 correction success is achieved when **100%** of audited first-run, profile, help, and session-context surfaces use **Crew Interaction Profile** framing with the labels **Product Strategy**, **UX/UI Design**, **Software Architecture**, and **AI Delivery Planning**, distinguish the profile from internal persona lenses, and preserve **100%** legacy `user-profile.yml` fixture compatibility without key migration or `/speckit.specify` depth-behavior regression.
- **SC-008**: Durable-instruction and multi-developer safety success is achieved when **100%** of audited shared instruction surfaces point to the current-user profile loader/path rule instead of concrete dial values, **0** shared-repository artifacts persist resolved per-developer settings, and paired-developer validation proves different local `user-profile.yml` files can produce different session guidance in the same repository without shared repo changes.
...
- **Project-level Crew Interaction Profile overrides** (Iteration 003 implements user-level only; project-level overrides remain future scope).
...
- **Hard role/gate-specific application of Crew Interaction Profile outside `/speckit.specify`** (this release only adds soft session-context guidance elsewhere).
- **Persisting resolved per-developer Crew Interaction Profile values into shared repository files or durable shared instructions**.
...
- **Decision**: Treat F-049 as a **five-iteration** feature. Preserve Iterations 001-003 as closed, keep Iteration 004 reserved for Proposal 120's full five-pillar bypass-detection scope, and add Iteration 005 for Proposal 141's bounded **Crew Interaction Profile / Persona Lens Separation** correction.
...
#### Crew Interaction Profile framing

- **Question**: How should Proposal 141 frame the user-facing profile model?
- **Decision (Authoritative)**: The user-facing model is a **Crew Interaction Profile**. All four visible decision areas are **Product Strategy**, **UX/UI Design**, **Software Architecture**, and **AI Delivery Planning**. The profile tells Specrew how much to ask, explain, recommend, and auto-decide; it is not a job-title identity claim.
...
- **Decision (Authoritative)**: The spec fixes the external contract rather than a storage redesign: every user-facing profile surface must render the four user-facing labels above, those labels must map deterministically to the existing persisted keys and internal lenses, and planning/implementation must preserve this mapping without changing schema keys or persona IDs.

#### Session-context application boundary

- **Question**: How broadly should the resolved current-user profile apply in Iteration 005?
- **Decision (Authoritative)**: Surface the resolved current user's Crew Interaction Profile in session context as **soft collaboration guidance for all agents**. It may shape phrasing, explanation depth, recommendations, and auto-decision posture during human-facing interactions, but `/speckit.specify` remains the only **hard-applied** behavior in this release.

#### Durable shared instruction rule

- **Question**: What may shared durable instructions store about the user profile?
- **Decision (Authoritative)**: Shared durable instructions must store the **rule** for loading the current user's profile from the user-level path (or loader), not concrete dial values. This preserves context durability without leaking one developer's local settings into shared repository artifacts.

#### Multi-developer safety

- **Question**: How should the spec handle multiple developers in the same repository?
- **Decision (Authoritative)**: Different developers may keep different local `user-profile.yml` files and receive different resolved session guidance with **no shared repository changes**. Multi-developer safety is mandatory for this slice.
...
- **Capacity Model**: 54-66 SP total across 5 iterations (Iteration 005 adds a bounded 6-8 SP **Crew Interaction Profile / Persona Lens Separation** slice on top of the approved Iteration 004 reservation):
  ...
  - **Iteration 005**: 6-8 SP (Proposal 141 Crew Interaction Profile / Persona Lens Separation, stable-key compatibility proof, soft session-context guidance, durable loader/path instruction rule, hard `/speckit.specify` boundary clarification, and multi-developer safety)
  - **Remaining Capacity**: Reduced by 6-8 SP versus the prior four-iteration model because Iteration 005 is an approved follow-on correction slice.
- **Roadmap Truth**: Iterations 001-003 are complete; Iteration 004 remains the reserved Proposal 120 slice unchanged; Iteration 005 is the approved Proposal 141 **Crew Interaction Profile / Persona Lens Separation** lane.
