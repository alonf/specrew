...
| Session-context soft guidance (FR-038/FR-040) | PASS at helper level | `New-CrewInteractionProfileSessionContext` returns `shared_project_truth=false`, `scope=current-user-runtime-guidance`, application = "soft … hard-applied only in /speckit.specify", decision areas keyed by display label, expertise_dials keyed by stable persona IDs. **Caveat:** verified by direct helper invocation + `specrew-start.ps1` parse-check; the in-situ start-context.json generation line was not exercised end-to-end (interactive first-run path). To be exercised during beta install. |
| Multi-developer independent resolution (FR-041 / SC-008 clause 1) | PASS | `dev-a.yml` (architect 9) and `dev-b.yml` (architect 2) resolve independently from their own local files; divergent profiles coexist. |
| No persisted per-developer values in shared repo (SC-008 clause 2) | PASS | `.specrew/start-context.json` + `.specrew/last-start-prompt.md` confirmed untracked (`git ls-files` empty). A `git grep` for `<decision-area-label>: <1-10>` across tracked README/docs/`.github`/`.claude`/`.agents`/`.specrew`/`extensions`/`.specify` surfaces returns **only** the `/specrew-user-profile` SKILL.md "Example output" fenced block (illustrative format, not a resolved developer profile). No shared artifact persists a developer's resolved dial values. |
