...
- Q7 (added 2026-05-23 after user surfaced enterprise-pricing gap): Organizations frequently have negotiated rates that differ from public list pricing (enterprise contracts, committed-spend discounts, time-limited promotions, batch/cache discounts, internal cross-billing rates). Should F-041 catalog read public rates AND support an `.specrew/pricing-overrides.yml` overlay where the org puts negotiated rates? → **Default A: Yes — overlay file pattern.** Public catalog stays the source of truth for what models exist + their public rates; `.specrew/pricing-overrides.yml` is a separate (gitignored by default) file the org maintains. Read precedence: per-model exact-match wins, then per-host discount factor, then catalog public rate. Each cost record carries both `effective_cost_usd` and `public_cost_usd` plus `pricing_source` attribution. Contract expiry warnings surface in dashboard.
...
| FR-017 | F-041 MUST support an optional `.specrew/pricing-overrides.yml` overlay file. When present, override values WIN over public catalog rates at cost-computation time. Read precedence per (host, model): (1) `pricing_overrides.hosts[host].model_overrides[model]` exact match, (2) `pricing_overrides.hosts[host].input_discount_factor` × catalog rate, (3) catalog's public rate (no override applies). Missing overrides file → falls back to public catalog (current behavior preserved). Per Q7 clarify default |
| FR-018 | Every cost record written by F-042 (per Proposal 070 cost.yml schema) MUST carry: `effective_cost_usd` (actually charged), `public_cost_usd` (what the catalog says), `pricing_source` (`public-list` / `org-negotiated` / `manual-override`), and `override_contract_id` (e.g., "ATC-2026-Q1") when an org-negotiated override applied. Enables audit ("65% of cost was at negotiated rates; 35% at public list") |
| FR-019 | `.specrew/pricing-overrides.yml` MAY include `effective_date` + `expiry_date` fields. When current date is within 14 days of `expiry_date`, `specrew where` dashboard MUST surface a warning ("ATC-2026-Q1 contract expires in 14 days; renew or rates revert to public list") |
| FR-020 | F-041 MUST ship a fixture pricing-overrides template at `templates/pricing-overrides-example.yml` with documented schema + worked examples. `specrew init` does NOT copy this to greenfield projects (default = public list pricing); users opt in by copying the template to `.specrew/pricing-overrides.yml` when they have negotiated rates. Specrew SHOULD add `.specrew/pricing-overrides.yml` to `.gitignore` on greenfield bootstrap to prevent accidental commit of negotiated rates |
| FR-021 | Catalog v2 schema MUST include per-model reasoning-effort metadata: `supports_reasoning_effort: bool`, `effort_levels: [string]` (subset of low/medium/high/xhigh/max), `default_effort: string`, `effort_token_multipliers: { low: 1.0, medium: 2.5, high: 5.0, xhigh: 7.5, max: 10.0 }` (approximate; refined via reconciliation in Proposal 106). Per Q8 clarify default |
| FR-022 | F-041 lean profile MUST route TWO dimensions per role: model tier AND reasoning effort. Per Q8 default mapping: Implementer = (cheap tier, low effort); Retro Facilitator = (cheap, low); Planner = (balanced, medium); Reviewer = (premium, high); Spec Steward = (premium, high); AI Researcher = (balanced, medium). Intake tasks always stay at premium tier with high effort regardless of profile (per FR-005); intake quality determines spec quality downstream |
| FR-023 | Per-host effort injection MUST dispatch on the host's native primitive: copilot host = model-config toggle (Copilot CLI exposes this per model where supported); claude host = `--effort <level>` launch flag (preferred) or `/effort` session command (fallback) or per-subagent frontmatter (when Slice 3 deploys subagents); codex host = `model_reasoning_effort = "<level>"` in `~/.codex/config.toml` or `--config 'model_reasoning_effort="<level>"'` launch arg or per-agent .toml; antigravity = deferred (handled in Antigravity slice). When a model doesn't support reasoning_effort (per catalog `supports_reasoning_effort: false`), the routing layer silently skips effort injection for that role |
| FR-024 | Routing decision entries in `.squad/decisions.md` (per FR-007) MUST record BOTH model AND effort: e.g., `"Implementer → claude-haiku-4-5 @ effort:low"`. Cost record schema (consumed by F-042 cost.yml) MUST include `effort_level` field so F-042's cost estimator can apply the `effort_token_multipliers` from the catalog when computing estimated cost |
| FR-025 | Explicit human overrides for effort (mirror pattern of FR-010 model overrides): if `.squad/config.json` has `roleEffortOverrides.<role>` set, that value wins over lean-profile default. Override source recorded as `human-config` in routing entry per FR-007 |
