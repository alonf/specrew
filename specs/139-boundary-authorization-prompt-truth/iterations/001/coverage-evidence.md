...
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tests\integration\multi-host-launch-path.tests.ps1` | pass after D-008 repair commit `6507c6af` | Proves version + host + runtime truth for initial/resume orientation across Codex, Claude, and Copilot/Squad, plus adapter-rendered interaction guidance. |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tests\integration\start-command.ps1` | pass after D-008 repair commit `6507c6af` | Checks actual generated `.specrew\last-start-prompt.md` and `.specrew\start-context.json` for `specrew_version`, `selected_host`, `runtime_class`, and host-rendered interaction guidance. |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tests\manual\copilot-squad-smoke.ps1` | pass after D-008 repair commit `6507c6af` | Release smoke now scans emitted prompt/orientation for missing version truth and false hard-coded host/runtime claims. |
| `pwsh -NoProfile -ExecutionPolicy Bypass -File tests\unit\boundary-authorization-prompt-truth.tests.ps1` | pass after D-008 repair commit `6507c6af` | Confirms core Rule 53 defines the response contract and delegates host-specific rendering without stale Squad automation wording. |
...
| D-008 version truth and host interaction rendering | yes | yes | yes | yes | No gap after repair. The generated prompt now includes installed Specrew prerelease truth, selected host, runtime class, lifecycle position, and host-adapter interaction guidance; Codex/Claude structured primitives and Copilot textual fallback are covered. |
