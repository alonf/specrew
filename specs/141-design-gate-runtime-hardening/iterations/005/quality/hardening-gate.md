...
**Post-Implementation Verification**: recorded — the three runtime-evidence concerns are verified: error-handling = graceful `@()` / "none available" on a missing lens file/section or absent answers JSON (extractor + render + coverage tests); test-integrity = SC-016 determinism + explicit-marker grandfather + bypass-closed (no marker + no `Addressed:` → FAIL) + placeholder-fails + no-op (`tests/unit/lens-applicability-selector.tests.ps1` 38/0, `tests/unit/design-analysis-gate.tests.ps1` 38 assertions / 14 checkpoints); operability = LLM/network-free pure functions + `index.yml` purity (test-asserted) + EXPLICIT-marker grandfather safety (Iteration 4 carries `fr026_grandfathered`; enforcement is the default so deleting `Addressed:` lines from an FR-026-era artifact FAILS) + honest anti-omission framing (the gate string + docs do not overclaim). The dogfood converged and the delete-the-`Addressed:`-lines discriminator PASSED. Validator iteration-005 PASS. See review.md + coverage-evidence.md.
...
| `operational-resilience-concerns` | `operability` | `addressed` | `runtime-evidence` | `recorded` | Enforcement MUST be LLM/network-free (only the recorded answers + the authored coverage entries are inputs); it MUST keep `index.yml` PURE; it MUST grandfather pre-FR-026 artifacts via an EXPLICIT marker (enforce-by-default; not inferred from missing `Addressed:` lines) so no silent bypass and no retroactive failure of Iteration 4; and it MUST NOT overclaim — the gate is anti-omission, not a quality guarantee. | `true` | FR-026/FR-010 require deterministic, decoupled, bounded, backward-compatible enforcement; operational correctness = no network/LLM, pure `index.yml`, explicit-marker grandfather safety (bypass closed), and honest framing. Promoted to runtime-evidence after implementation. | `—` |
...
- FR-026 must grandfather pre-FR-026 artifacts via the EXPLICIT `fr026_grandfathered` marker (Iteration 4 carries it): re-validating Iteration 4 must NOT fail, while enforcement stays the default for unmarked FR-026-era artifacts (deleting `Addressed:` lines must FAIL, not no-op).
