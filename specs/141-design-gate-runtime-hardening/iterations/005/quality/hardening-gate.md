...
| `error-handling-expectations` | `robustness` | `addressed` | `planning-time-analysis` | `pending-post-implementation` | The extractor + render + coverage gate MUST fail safe: a missing lens file or absent `## Design Decision Points` section yields `@()` / "none available" (never throw); a missing/empty `lens-applicability.json` makes the gate no-op; a malformed artifact does not corrupt `design-analysis.md`. | `true` | The defect class is missing/partial lens inputs; the control is fail-safe defaults covered by positive + negative tests (T005). Promoted to runtime-evidence after implementation. | `—` |
...
- The three `addressed` concerns are `planning-time-analysis` at planning time (Runtime Evidence Status `pending-post-implementation`); they are promoted to Evidence Basis `runtime-evidence` / Runtime Evidence Status `recorded` at review-signoff once the tests + dogfood run (mirrors the Iteration 4 closure pattern).
