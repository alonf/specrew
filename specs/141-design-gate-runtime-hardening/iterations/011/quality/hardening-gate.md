...
- The three `addressed` concerns are promoted to `runtime-evidence`/`recorded` on the strength of the T001
  SC-026 floor + the wiring test through the real gate entry + the T005 presence-locks — the deterministic
  controls are verified (the tests are green). The BEHAVIORAL payoff — the agent stops manufacturing agreement
  on the Squad host — is NOT unit-provable and remains the SC-027 re-dogfood.
