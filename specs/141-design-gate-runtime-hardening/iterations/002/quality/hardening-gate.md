...
**Pre-Implementation Readiness**: Iteration 002 is a bug-fix / small-fix slice — start-packet correctness (FR-011 empty `specs//` paths, FR-014 host-wording leak) plus two non-blocking smoke-2 cleanup items. Design-analysis is not required (defect repair, no architectural fork). No release/publish; no Unix/wrapper/bootstrap surfaces.
...
| `operational-resilience-concerns` | `operability` | `not-applicable` | `not-applicable` | `not-needed` | `—` | `false` | No server, SLO, telemetry, oncall, or operational dependency; the change is in the generated start-prompt and the gate harness output. | `—` |
