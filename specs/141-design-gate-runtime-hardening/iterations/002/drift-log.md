...
- **T004 (gate-harness trailing `$LASTEXITCODE` / quality-prereq path) — verified ALREADY CLEAN, no code change (maintainer-approved disposition 2026-06-02).** Reproduce-first found the iteration-1 smoke symptoms are not produced by any committed code path: `GATE_VALID: True` exists nowhere in source (it was a manual echo a prior coordinator improvised during the smoke), and `Invoke-SpecrewDesignAnalysisPlanBoundaryGate` returns `Valid=True` with `$LASTEXITCODE=0` and no stray error on a valid artifact (verified on 141/001 and in a unit fixture). Both quality/prereq command paths the generated guidance references (`check-prerequisites.ps1`, `resolve-quality-profile.ps1`) exist. Closed with a durable guard test in `tests/unit/design-gate-runtime-hardening.tests.ps1` (asserts clean `$LASTEXITCODE` + path resolution); this also satisfies T005's clean-harness-exit coverage.
