...
| T004 | Create `scripts\internal\coordinator-prompt-surgery.ps1` — `Get-SpecrewCoordinatorPromptForHost -Host <kind> -ExistingPrompt <str>` returns body with header swap + rules 12/35/37/42-44 stripped for non-Squad hosts; copilot path returns input unchanged (byte-identical) | Implementer | 2 | T001 | prompt-surgery tests: copilot path is byte-identical; non-Squad path has new header + 4 rules stripped + everything else unchanged |
...
| T006 | Rewrite the load-bearing dispatch literal at `scripts\specrew-start.ps1:3131` to use `Get-SpecrewHostLaunchInvocation`; preserve Windows `Start-Process pwsh` + Linux `SPECREW_DEFERRED_LAUNCH_FILE` branches; copilot path stays byte-identical | Implementer | 2 | T005 | Copilot regression test: argv on Windows + Linux match pre-F-040 verbatim |
...
| AC11 | FR-011 (coordinator-prompt surgery) | T004, T005 |
| AC12 | Antigravity + `--host auto` deferred-guidance rejection | T001 |
| AC13 | Cross-platform parity (Windows + Linux) | T006, T009 |
