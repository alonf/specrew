...
| FR-014 | When the selected host is Codex, Specrew MUST extend the coordinator-prompt body (via the FR-012 directive surgery) with **explicit boundary-advance instructions in pwsh form** rather than slash-command form. Example: replace `"To advance the plan boundary, invoke /speckit.specrew-speckit.sync-plan"` with `"To advance the plan boundary, run: pwsh -File .specify/extensions/specrew-speckit/scripts/sync-boundary-state.ps1 -BoundaryType plan"`. This compensates for Codex's no-slash-command limitation so the agent can still discover and invoke F-039's canonical authorization-gating path |
| FR-015 | F-040 MUST document the **cooperative-enforcement nature** of F-039 across multi-host launches in `docs/user-guide.md`: F-039 fires only when the agent invokes Specrew's canonical sync path (slash command on Copilot/Claude, pwsh direct invocation on Codex). Runtime enforcement (host-layer interception of any write to boundary state) is **out-of-scope for F-040** and tracked in [Proposal 105 — Host-Native Hook Deployment](../../proposals/105-host-native-hook-deployment.md). Strict-mode users should prefer Copilot or Claude over Codex until Proposal 105 ships |
...
- **Host-native HOOKS deployment** — Claude Code has rich hooks (PreToolUse, PostToolUse, SubagentStart, Stop, TaskCreated, TaskCompleted) configured in `.claude/settings.json`. Antigravity has hooks too. Copilot has none. F-040 does NOT deploy host-native hooks. **Tracked as [Proposal 105 — Host-Native Hook Deployment for Runtime Boundary Enforcement](../../proposals/105-host-native-hook-deployment.md)** (drafted alongside F-040 on 2026-05-23). When Proposal 105 ships, F-039 enforcement on hook-supporting hosts is elevated from cooperative to runtime-enforced; F-040's launch-path work stays intact. For F-040, boundary enforcement stays at the Specrew-authored slash-command + on-disk-state layer, which works uniformly across all hosts albeit cooperatively
...
- **105 Host-Native Hook Deployment** (NEW — drafted 2026-05-23 alongside F-040) — composes with F-040 to elevate F-039 enforcement from cooperative to runtime on hook-supporting hosts (Claude, Antigravity). F-040 ships cooperative-only; 105 layers runtime enforcement on top
...
- **F-039 Launch-Mode Boundary Enforcement (shipped)** — F-040 honors the new boundary-authorization helpers; per-host launch invocations are governed by the same boundary discipline as Copilot launches. F-039's enforcement is COOPERATIVE on all hosts after F-040; Proposal 105 elevates this to runtime on hosts with hook surfaces
