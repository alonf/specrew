...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F1 | actionable defect | Top-level `--version` / `-v` parity gaps | FR-001, SC-001 | 001 | Implemented and verified parity in US1. | `tests/integration/validate-versions-cli-behavior.ps1` passing + `scripts/specrew.ps1` diff | Implementer | done |
| F2 | actionable defect | False warning when version is determinable | FR-002, SC-002 | 001 | Fixed warning guard conditions in US1. | `tests/integration/validate-versions-cli-behavior.ps1` passing + `scripts/specrew-version.ps1` diff | Implementer | done |
| F3 | actionable defect | `specrew start` missing catalog auto-repair gap | FR-004, SC-003 | 001 | Added and verified startup auto-repair. | `tests/integration/start-recovery-flow.tests.ps1` passing + `scripts/specrew-start.ps1` diff | Implementer | done |
| F4 | actionable defect | `specrew init` gap handling exits early | FR-005, SC-003 | 001 | Route force and non-force paths to deployment flow for catalog gaps. | `tests/integration/start-recovery-flow.tests.ps1` passing + `scripts/specrew-init.ps1` diff | Implementer | done |
