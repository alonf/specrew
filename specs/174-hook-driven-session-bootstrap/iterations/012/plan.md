...
(accurate cross-host handover), plus the DR-3 host-confirmation honesty fix: Claude/Codex/Copilot confirmed for
hook bootstrap + rolling handover read/write, Cursor pending, rich packet/verdict capture Claude-only.

## Tasks

| Task | Title | Requirement | Story | Effort | Owner | Owner File Globs | Status | Agent | Actual | Verdict |
| ---- | ----- | ----------- | ----- | ------ | ----- | ---------------- | ------ | ----- | ------ | ------- |
| T001 | README reconcile — flip the body to the hook-driven model: `init` then launch your host (no `specrew start`); rewrite "Switch your AI host mid-feature" to cross-host auto-resume via the rolling handover; add `.specrew/handover/session-handover.md` to durable artifacts; fix the host-confirmation claim to evidence-scoped wording: Claude/Codex/Copilot confirmed for hook bootstrap + rolling handover read/write, Cursor pending, rich packet/verdict capture Claude-only (DR-1/DR-2/DR-3) | FR-006, FR-009 | US-1 | 3 | Implementer | `README.md` | planned | TBD | | |
...
| T007 | Verification — markdownlint + doc token-parity (wrapper-docs-parity) green; re-run the doc-coverage assessment (or equivalent check) to confirm no HIGH gaps + no stale/false new-model claims remain, including the updated real-host evidence scope for Claude/Codex/Copilot/Cursor and the Claude-only rich-packet/verdict-capture caveat (SC-1..SC-6) | SC-001 | US-3 | 2 | Implementer | `tests/**`, `docs/**` | planned | TBD | | |

Total planned: 16 SP (≤ 20 cap). Defer priority if it overruns: T006 announcement/polish first. Do **not** defer
the DR-3 false-claim removal in T006, and do **not** defer T005 while SC-4 requires the handover file schema plus
the `specrew handover author` and `specrew hooks` command docs. Never defer the two MUSTs (T001/T002/T003) or the
honesty fix.
