...
- **FR-011 — Empirical handover gate**: The completed real-host spike selected outcome 2: the pinned build's Stop payload has no assistant-message field; `--export` writes ATIF before Stop; and an in-package ATIF normalizer MUST emit the existing Claude-like JSONL turn shape consumed by the unchanged parser. Prerelease promotion MUST rerun this bounded canary.
...
- The real-host spike selected FR-011 outcome 2. Tested build `2026.7.23 (3bd47f77)` emitted a Stop payload containing only `hook_event_name` and `stop_hook_active`; `--export` wrote ATIF before Stop; Devin-local normalization to the existing Claude-like JSONL shape was read successfully by the unchanged parser.
- The same spike found a pinned-build Windows hook-runner constraint: Claude-format commands fail with `program not found` unless `sh.exe` is on `PATH`, despite Devin's general Windows non-interactive shell being PowerShell. Devin remains experimental until the integration handles or explicitly documents this prerequisite and prerelease evidence revalidates it.
