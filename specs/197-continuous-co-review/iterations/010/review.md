...
- Downstream-found budget drift (consumer project tesr197local, 2026-07-09 — the first-ever exercise of the --live door's default budget): the manual door kept iteration-002's hardcoded 120s that the auto path had already raised to 300s+config, starving agentic reviewers (agy cut mid-review with salvaged notes). Maintainer fix-now: the door now resolves explicit flag → co_review_timeout_seconds → 300s baseline (the exact auto-path resolution); Test 5 regression assertion on the status envelope. Recorded as D-197-I010-006: fixed-now
- SC-022 harness breadth (copilot/cursor-agent installed-but-unauthorized on this machine): the antigravity leg of this defer was DISCHARGED in-iteration (installed+authorized 2026-07-08, exercised end-to-end as the independent reviewer — see quality/cross-host-validation.md); the remaining copilot/cursor-agent breadth stays deferred per the N8 installed+authorized scope rule and the maintainer decision recorded in .squad\decisions.md (copilot headless vector since probe-validated, NOTE-197-I010-005 — one catalog row away): deferred
- Codex auto-fire f1 — evidence recorder persists caller-supplied numbers (forgeable by the implementer path): interim honesty fix SHIPPED (prompt says implementer-recorded, spot-checks armed as blocking forgery detection); the runner-observed wrapper is Proposal 203 W8 per DEC-197-I010-008: deferred
- Codex auto-fire f2 — navigator fires without BaselineRef so checkpoint reviews re-read the whole feature diff (safe-but-slow, vs FR-027's incremental intent): per-checkpoint baseline threading is Proposal 203 W9 per DEC-197-I010-008: deferred
- Codex auto-fire f3 — fire-time dedup digest not threaded to the detached child (edits between fire and child-digest reviewed as "the checkpoint"; certified==reviewed holds, bookkeeping drift only): fire-time digest freeze is Proposal 203 W10 per DEC-197-I010-008: deferred
...
- Drift: 6 events + 1 carried-finding closure, all resolved — see drift-log.md.
