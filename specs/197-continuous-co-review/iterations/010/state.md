...
- THIRD FINDING RESOLVED 2026-07-09: a navigator AUTO-FIRE (codex, run 20260708T225439577, round 1, unprompted on the proposals checkpoint) caught the D-004 follow-on — auto-fire dedup still keyed on HEAD^{tree}, deduping dirty increments as already-reviewed (gate freshness backstop held; coverage gap only). Maintainer "1" = fix now (DEC-197-I010-006): checkpoint identity = the certified digest for dedup + pending registration (`Get-ContinuousCoReviewCheckpointIdentity`, HEAD fallback); content-addressed regression tests; CCR suite 257/257.
- FOURTH FINDING RESOLVED 2026-07-09 (downstream-found, consumer project tesr197local): the --live door's default budget was iteration-002's hardcoded 120s (the auto path was fixed to 300s+config back then; the manual door drifted) — starved agentic reviewers on the first-ever default-budget exercise. Maintainer "1" = fix now (DEC-197-I010-007, drift D-197-I010-006): door resolves explicit flag → co_review_timeout_seconds → 300s baseline; Test 5 regression assertion. review-command suite fully PASS.
