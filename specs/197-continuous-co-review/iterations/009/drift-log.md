...
### D-197-I009-011 — `specrew init` passed an unsupported `--force` to Spec Kit 0.8.4's `integration install`; native host commands silently never installed

**Status**: FIXED this iteration (dropped the `--force`)
**Detected by**: F-197 rehearsal setup 2026-06-30 — `specrew init` logged `[warn] specify integration install claude failed … Usage: specify integration install [OPTIONS] KEY` (and the same for `agy`).

**Drift + root cause**: file:///C:/Dev/197-continuous-co-review/scripts/specrew-init.ps1 called `specify integration install <key> --force`, but Spec Kit **0.8.4** — the pinned + min-supported version — `integration install` takes only `[--script | --integration-options] KEY`; there is **no `--force` flag**. So Spec Kit rejected the WHOLE call with a usage error and the native host command surface (`.claude/skills/speckit-*`, `agy`) silently never installed. Non-fatal (palette hosts fall back to the governed scripts), but it leaves a host told to use `/speckit.*` commands it does not have (the #2884 4th-face class the loop was added to prevent). The DEEPER cause (surfaced once the re-init got past the usage error): Spec Kit 0.8.4 is **single-integration** — `specify init --ai copilot` installs copilot, and the subsequent `integration install claude` is refused with "Integration 'copilot' is already installed." `--force` was the intended override for exactly that, but 0.8.4 has no `--force`. So on 0.8.4 the palette-host native surfaces genuinely cannot be added beside copilot — a Spec Kit capability limit, not a Specrew defect.

**Fix (two parts)**: (1) dropped `--force` (`specify integration install --help` on 0.8.4 confirms it is not a valid option) — removes the usage-error rejection; (2) classified the resulting "already installed" (the single-integration limit) as an EXPECTED skip — logged as a clean `[info]` ("Spec Kit installs one integration per project; <host> uses the governed-scripts fallback"), not an alarming `[warn]`. So a fresh `specrew init` is now clean. The native claude/agy `/speckit.*` surfaces still cannot install on 0.8.4 (the Spec Kit limit), but that is the designed governed-scripts fallback, and co-review (the demo) does not depend on it. If native claude commands are ever wanted, init with claude as the PRIMARY Spec Kit integration (`specify init --ai claude`) instead of copilot — a separate init-design change. (The Spec Kit Python CLI `UnicodeEncodeError`s / hangs under captured non-UTF-8 stdout — why end-to-end verification runs in a real UTF-8 console, not here.)

**Related observation (candidate follow-up, not fixed here)**: the Spec Kit Python CLI crashes with `UnicodeEncodeError` when stdout is a non-UTF-8 captured stream (it emits emoji/box-drawing). Specrew's `Invoke-NativeCommandForOutput` could export `PYTHONIOENCODING=utf-8` / `PYTHONUTF8=1` when invoking `specify` to harden init in CI and non-UTF-8 consoles.

**Trace**: governance / Spec Kit integration (init correctness); not an F-197 requirement.
