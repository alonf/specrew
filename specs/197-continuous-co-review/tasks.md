...
- [ ] T091 [owner: Architect] [sp: 3.0] [R5/N1] **Hard-timeout completion** — route the inline `specrew review --live` spawn through the isolated-task supervisor; instrument the live escape first; delete `worktree-reviewer.ps1`'s duplicate inline `$proc.Kill($true)`; WSL-validated (hard gate) (Trace: FR-037, SC-024, NFR-001; owns: `worktree-reviewer.ps1`, `agent-tasks/isolated-task-supervisor.ps1`, `agent-tasks/isolated-task-launcher.ps1`, `tests/continuous-co-review/**`)
- [ ] T093 [owner: Implementer] [sp: 1.5] [R3] **Host-independence fallback** — pre-flight independence check; labelled same-host fallback (never block); the answer upgrades the next run; honour-or-surface `--host` (Trace: FR-035, FR-016, SEC-004; owns: `reviewer-selection-policy.ps1`, `reviewer-authorization-gate.ps1`, `tests/continuous-co-review/**`)
- [ ] T094 [owner: Reviewer] [sp: 2.5] [R4] **Tiered degraded-evidence gate** — 3-dimension label (completeness/independence/budget); full+independent auto; partial/same-host need a recorded first-class ack; never deadlock; degraded-block override with recorded reason (Trace: FR-036, SC-019, SC-020, SC-024; owns: `review-signoff-evidence-gate.ps1`, `inline-review-gate-evaluator.ps1`, `tests/continuous-co-review/**`)
...
- [ ] T099 [owner: Implementer] [sp: 1.5] [R8] **Material-turn conformance gate** — run the Stop-hook conformance parse only after a material turn, so trivial stops stay cheap (Trace: FR-040, SC-025; owns: `specrew-conformance-provider.ps1` + `.specify` mirror, `tests/continuous-co-review/**`)
- [ ] T100 [owner: Architect] [sp: 4.0] [R7/N2] **OS-native robust supervisor** — Windows Job Object (`KILL_ON_JOB_CLOSE`) + Unix `setsid`+PGID group kill so the whole reviewer tree dies atomically; session-scoped pidfile registry + SessionStart orphan reaper; record `terminal_reason`; WSL-validated (Trace: FR-039, SC-025, NFR-001; owns: `agent-tasks/isolated-task-supervisor.ps1`, `agent-tasks/isolated-task-launcher.ps1`, `agent-tasks/process-tree.ps1`, `tests/continuous-co-review/**`)
...
- [ ] T107 [owner: Implementer] [sp: 2.0] [N5] **code-review-agent.md preservation fold** — graft the TO-FOLD manifest rows (falsification, P145 phases, per-lens, workshop-conformance, claim/design-trace, deterministic-failure≠clean, secret-non-exfiltration) into `Get-ContinuousCoReviewSlimPrompt`; drop the DROP rows; retire the file to a reference doc; re-point the test (Trace: FR-017, FR-018, FR-021, SEC-007, SC-013, SC-014; owns: `worktree-reviewer.ps1`, `code-review-agent.md`, `tests/continuous-co-review/contracts/reviewer-instruction.Tests.ps1`) — acceptance: preservation manifest (requirement-reconciliation.md §D)
- [ ] T108 [owner: Implementer] [sp: 1.5] [N6] **Codex reliability** — adapter retry-once on a 0-byte result before `no-parseable-findings`; diagnostic on the empty exit-0; a still-empty retry fails loudly (never false-green) (Trace: FR-033, SC-024; D-197-I009-015; owns: `reviewer-host-catalog.ps1`, `worktree-reviewer.ps1`, `tests/continuous-co-review/**`)
- [ ] T109 [owner: Reviewer] [sp: 1.5] [N7] **Flush-race forensic** — forensic test confirming/refuting the conformance stop-block flush/read race on real data; cheaper mitigation only if confirmed, else close refuted-with-evidence (Trace: FR-040, SC-025; D-197-I009-003; owns: `specrew-conformance-provider.ps1`, `tests/continuous-co-review/**`)
- [ ] T110 [owner: Spec Steward] [sp: 2.0] [N8] **Cross-host validation** — exercise the Stop-hook co-review fire across the installed+authorized harnesses (claude, codex today); record which were exercised vs unavailable (Trace: SC-012, SC-022; owns: `specs/197-continuous-co-review/iterations/010/**`)
