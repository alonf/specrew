...
| 010 | **Completes the robustness charter — T091-R5, T093, T094, T096, T099, T100, escalation-latch wiring, `code-review-agent.md` fold, D-197-I009-003/-015, SC-012/022 (all 11 tasks, 24/24 SP, 2026-07-08)** | **reviewing** (implementation complete; codex escalation f1/f2 fixed maintainer-approved; verification round in flight; review-signoff next) | maintainer host-neutrality finding D-197-I010-002 fixed same-day; D-197-I009-003 closed refuted-with-evidence; T108 retry fired on real codex empty-exit0 twice |
...
- **Collision:** the iter-008 "Dogfood Repair Addendum" reuses **`T083/T084/T085`**, which iter-006 already used (`18557e80`, `6a24838b` — commit-cited = authoritative). The addendum is now renumbered to **`T087/T088/T089`** by iteration-009 T095 (done); iter-006's commit-cited `T083-T086` stay canonical.
...
- [X] T095 [owner: Spec Steward] [sp: 0.5] [governance — non-requirement] **Resolve the T083–T085 collision** — renumber the iter-008 Dogfood Repair Addendum to T087–T089 (iter-006's commit-cited T083–T086 stay canonical) (Trace: governance hygiene — not requirement-driven; explicitly exempt from FR/SC traceability)
...
- [x] T097 [owner: Implementer] [sp: 2.0] [R7] **Zero-inheritance detached spawn + auto-budget revert** — spawn the detached review via `Win32_Process.Create` (Windows: `bInheritHandles=FALSE` + reparented + `ShowWindow=SW_HIDE`) so it cannot hold the dispatcher's — and transitively the HOST's — stdout pipe (the multi-minute Stop); the first `-11/-12` handle-clear was INSUFFICIENT (Start-Process inherits EVERY handle), proven by a 4-level host→dispatcher→provider→review harness (drift-log D-197-I009-005); Unix keeps `Start-Process -Redirect*` (verified clean 2.8s); the detached-entry self-redirects its stdio; revert T092's AUTO generous-budget bump (auto uses the default; generous is manual-`--live` only) (Trace: FR-039, FR-040, SC-025; owns: `co-review-service.ps1`, `worktree-review-detached-entry.ps1`, `worktree-review-orchestrator.ps1`, `tests/continuous-co-review/integration/detached-spawn-no-block.Tests.ps1`)
...
## Iteration 010 — Robustness completion + reviewer-instruction fold — TASKS (approved for plan 2026-07-01; ship 0.40.0)

**Why:** the 2026-07-01 "finish it perfectly" decision. iter-009 delivered the graceful-degradation spine but carried R3/R4/R6 (planned-not-built) + R5 (needs-rework) to iter-010; the maintainer chose full scope (D2: include the Phase-2 supervisor T099/T100) in ONE closing iteration, shipping **0.40.0**. Design of record: [iterations/010/design-analysis.md](iterations/010/design-analysis.md) (Option A; T100 = OS-native atomic containment "OS base"; reviewer session-resume considered + rejected). iter-009's D1–D5 stand; new decisions are N1–N8. Cap raised to 26 (OVERRIDE 4, applied after iter-009 closed). Carried tasks keep their iter-009 IDs; new work is T106–T110. Bidirectional traceability below.

- [x] T091 [owner: Architect] [sp: 3.0] [R5/N1] **Hard-timeout completion** — route the inline `specrew review --live` spawn through the isolated-task supervisor; instrument the live escape first; delete `worktree-reviewer.ps1`'s duplicate inline `$proc.Kill($true)`; WSL-validated (hard gate) (Trace: FR-037, SC-024, NFR-001; owns: `worktree-reviewer.ps1`, `agent-tasks/isolated-task-supervisor.ps1`, `agent-tasks/isolated-task-launcher.ps1`, `tests/continuous-co-review/**`)
- [x] T093 [owner: Implementer] [sp: 1.5] [R3] **Host-independence fallback** — pre-flight independence check; labelled same-host fallback (never block); the answer upgrades the next run; honour-or-surface `--host` (Trace: FR-035, FR-016, SEC-004; owns: `reviewer-selection-policy.ps1`, `reviewer-authorization-gate.ps1`, `tests/continuous-co-review/**`)
- [x] T094 [owner: Reviewer] [sp: 2.5] [R4] **Tiered degraded-evidence gate** — 3-dimension label (completeness/independence/budget); full+independent auto; partial/same-host need a recorded first-class ack; never deadlock; degraded-block override with recorded reason (Trace: FR-036, SC-019, SC-020, SC-024; owns: `review-signoff-evidence-gate.ps1`, `inline-review-gate-evaluator.ps1`, `tests/continuous-co-review/**`)
- [x] T096 [owner: Implementer] [sp: 3.0] [R6] **Remediation menu** — one menu at the next Stop (more time / different host / narrow scope: code · process · file · function / accept partial / override), carried via `co-review-round-state.json`; honour the human-directed scope (Trace: FR-038, FR-024, FR-025; owns: `continuous-co-review-navigator.ps1`, `specrew-co-review-navigator-provider.ps1`, `tests/continuous-co-review/**`)
- [x] T099 [owner: Implementer] [sp: 1.5] [R8] **Material-turn conformance gate** — run the Stop-hook conformance parse only after a material turn, so trivial stops stay cheap (Trace: FR-040, SC-025; owns: `specrew-conformance-provider.ps1` + `.specify` mirror, `tests/continuous-co-review/**`)
- [x] T100 [owner: Architect] [sp: 4.0] [R7/N2] **OS-native robust supervisor** — Windows Job Object (`KILL_ON_JOB_CLOSE`) + Unix `setsid`+PGID group kill so the whole reviewer tree dies atomically; session-scoped pidfile registry + SessionStart orphan reaper; record `terminal_reason`; WSL-validated (Trace: FR-039, SC-025, NFR-001; owns: `agent-tasks/isolated-task-supervisor.ps1`, `agent-tasks/isolated-task-launcher.ps1`, `agent-tasks/process-tree.ps1`, `tests/continuous-co-review/**`)
- [x] T106 [owner: Implementer] [sp: 1.5] [N4] **Escalation-latch wiring** — load `escalation-latch.ps1` via `_load.ps1`; invoke from the navigator so a ceiling escalation surfaces once then latches quiet; reset round-state on a converged checkpoint; integration test on real transcript data (Trace: FR-029, NFR-005; owns: `escalation-latch.ps1`, `_load.ps1`, `continuous-co-review-navigator.ps1`, `tests/continuous-co-review/**`)
- [x] T107 [owner: Implementer] [sp: 2.0] [N5] **code-review-agent.md preservation fold** — graft the TO-FOLD manifest rows (falsification, P145 phases, per-lens, workshop-conformance, claim/design-trace, deterministic-failure≠clean, secret-non-exfiltration) into `Get-ContinuousCoReviewSlimPrompt`; drop the DROP rows; retire the file to a reference doc; re-point the test (Trace: FR-017, FR-018, FR-021, SEC-007, SC-013, SC-014; owns: `worktree-reviewer.ps1`, `code-review-agent.md`, `tests/continuous-co-review/contracts/reviewer-instruction.Tests.ps1`) — acceptance: preservation manifest (requirement-reconciliation.md §D)
- [x] T108 [owner: Implementer] [sp: 1.5] [N6] **Codex reliability** — adapter retry-once on a 0-byte result before `no-parseable-findings`; diagnostic on the empty exit-0; a still-empty retry fails loudly (never false-green) (Trace: FR-033, SC-024; D-197-I009-015; owns: `reviewer-host-catalog.ps1`, `worktree-reviewer.ps1`, `tests/continuous-co-review/**`)
- [x] T109 [owner: Reviewer] [sp: 1.5] [N7] **Flush-race forensic** — forensic test confirming/refuting the conformance stop-block flush/read race on real data; cheaper mitigation only if confirmed, else close refuted-with-evidence (Trace: FR-040, SC-025; D-197-I009-003; owns: `specrew-conformance-provider.ps1`, `tests/continuous-co-review/**`)
- [x] T110 [owner: Spec Steward] [sp: 2.0] [N8] **Cross-host validation** — exercise the Stop-hook co-review fire across the installed+authorized harnesses (claude, codex today); record which were exercised vs unavailable (Trace: SC-012, SC-022; owns: `specs/197-continuous-co-review/iterations/010/**`)

**Traceability (bidirectional):** FR-035→T093, FR-036→T094, FR-037→T091, FR-038→T096, FR-039→T100, FR-040→T099+T109, FR-017/018/021+SEC-007+SC-013/014→T107, FR-029→T106, FR-033→T108, SC-012/022→T110. Every task traces to ≥1 FR/SC/finding; every in-scope requirement has ≥1 covering task. Capacity 24.00/26 (2.0 headroom). Full after-tasks check to run before before-implement.
