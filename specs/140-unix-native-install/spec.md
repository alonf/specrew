...
- **FR-014**: Documentation for macOS/Linux (`README.md`, `docs/getting-started.md`, `docs/user-guide.md`, `docs/troubleshooting.md`) MUST present native bash/zsh commands first — install via `install.sh` / `curl … | sh` from zsh/bash, then native `specrew …` usage — NOT `pwsh -File …` and NOT manual `Install-Module` as the primary path. The manual `Install-Module` path (run from inside `pwsh`) MUST be demoted to a clearly-labelled fallback, including a note that its PowerShell Gallery prompt defaults to `N` so plain **Enter declines** the install — friction the native `install.sh` path already avoids (it sets the PSGallery installation policy to Trusted and installs non-interactively with `-Force`, failing closed on a declined/failed install). PowerShell Core MAY be mentioned as an internal dependency but MUST NOT be presented as a manual prerequisite, except for unsupported platforms or a failed auto-install (where the manual dependency-install docs apply). Module / `pwsh -File` commands remain a documented fallback. Doc command examples MUST be parity-checked against the canonical registry where practical, or allowlisted with rationale. *(Extended 2026-06-02 from the macOS manual-smoke evidence — `macos-smoke-evidence.md`: a tester fell into the manual `Install-Module`-from-zsh + PSGallery-default-`N` trap because the native path was not the one they followed; these are **solved by the native install path** but MUST be addressed in docs.)*
...
- **FR-018** *(carved-out — delivery: separate `specrew init` slice)*: When `specrew init` dependency validation finds an outdated or shadowed Node.js on macOS, its diagnostics MUST call out the common **`nvm`-shadows-Homebrew-Node** case (a `brew install/upgrade node` that does not change the active `node -v` because the `nvm` shim precedes Homebrew on `PATH`) and MUST instruct the user to verify `node -v` from the environment Specrew actually runs in (`pwsh`), not only the login shell (`zsh`). macOS verification (automated or manual) MUST confirm `node -v` inside the Specrew runtime environment. **Delivery: a separate `specrew init` dependency-diagnostics slice** (carved out per maintainer decision 2026-06-02 to keep feature 140 scoped to wrappers / `install.sh` / docs). Feature 140's Iteration-3 macOS validation surfaces and documents this condition in troubleshooting docs but does NOT implement the diagnostic. *(Added 2026-06-02 from the macOS manual-smoke evidence — `macos-smoke-evidence.md`; NOT discharged by the Ubuntu Iteration-2 CI proof.)*
- **FR-019** *(carved-out — delivery: separate `specrew init` slice)*: When `specrew init` finds Spec Kit missing or below the supported floor, its diagnostics MUST print the exact remediation command `uv tool install specify-cli --from git+https://github.com/github/spec-kit.git@v<version> --force`, where `<version>` is derived from Specrew's **runtime supported-versions baseline** (`scripts/internal/supported-versions.yml` → `speckit.min`, currently `0.8.4`; `max_tested` `0.9.0`) — NOT a hardcoded literal that can go stale. The remediation MUST be documented in troubleshooting and validated. **Delivery: the same separate `specrew init` dependency-diagnostics slice as FR-018**; feature 140's Iteration-3 macOS validation surfaces and documents the condition but does NOT implement the diagnostic. *(Added 2026-06-02 from the macOS manual-smoke evidence; reconciled to the supported-versions baseline rather than a stale literal.)*

### Traceability & Governance Requirements *(mandatory)*

- **TG-001**: Each user story MUST map to one or more functional requirements. (US1 → FR-001/002/003/004/008/012; US2 → FR-005/006/007/012/015/016/017; US3 → FR-009/010/011; US4 → FR-013/014.) **FR-018 / FR-019 are carved-out follow-up requirements delivered by a separate `specrew init` dependency-diagnostics slice (see Out of Scope) and are intentionally NOT mapped to a feature-140 user story or iteration.**
- **TG-002**: Each requirement MUST identify expected owner role(s) at plan time (Implementer for wrappers/installer; Reviewer for parity + packaging; Spec Steward for docs honesty). FR-018/FR-019 owner roles are owned by the separate `specrew init` slice, not feature 140.
- **TG-003**: Each requirement MUST identify its intended iteration / delivery window at plan time (this feature is estimated 10-15 SP). Delivery windows: Iteration 1 = wrapper baseline; Iteration 2 = `install.sh` Ubuntu auto-install + parity CI (both closed); **Iteration 3** = FR-014 (extended, native-first docs), FR-017 (`--prerelease`), FR-015 macOS halves + the greenfield/brownfield release gate. **FR-018 / FR-019 delivery window = a separate `specrew init` dependency-diagnostics slice, NOT a feature-140 iteration** — so feature 140's per-iteration traceability is not obligated to carry them (reconciliation under TG-004 / Out of Scope).
...
- The macOS manual-smoke findings (`macos-smoke-evidence.md`) are **NOT discharged by the Ubuntu Iteration-2 CI proof**: the manual `Install-Module` / PSGallery-prompt friction is solved by the native `install.sh` path but MUST be addressed in docs (FR-014), while the Node/`nvm`-shadowing and old-Spec-Kit handling MUST be covered by Iteration-3 macOS validation (documenting the prerequisites) and implemented in the separate `specrew init` dependency-diagnostics slice (FR-018/FR-019). The Spec Kit floor used by diagnostics is the runtime baseline `scripts/internal/supported-versions.yml` (`speckit.min 0.8.4` / `max_tested 0.9.0`), not a hardcoded literal.
...
- **macOS `specrew init` dependency-diagnostic *implementation*** — the `nvm`-shadows-Homebrew-Node detection (FR-018) and the Spec Kit version-UX / supported-baseline-sourced upgrade command (FR-019). Carved to a **separate `specrew init` dependency-diagnostics slice** per maintainer decision 2026-06-02 to keep feature 140 focused on wrappers / `install.sh` / docs. Feature 140 covers only the native-first docs (FR-014), the macOS install proof, and *documenting* these prerequisites in troubleshooting — NOT the diagnostic code. *(TG-004 reconciliation: FR-018/FR-019 are recorded here as authoritative requirements with an explicit external delivery home, so they are not lost while remaining outside feature 140's implementation scope.)*
