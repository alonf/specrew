...
- **FR-006**: Wrapper installation MUST be idempotent and safe: dry-run / `-WhatIf` support, explicit overwrite behavior, no mutation outside the requested bin directory, and a clear warning when the bin directory is not on `PATH`. [NEEDS CLARIFICATION: default behavior when the bin directory does not exist — auto-create, or require `-Force`? And when not on `PATH` — warn-only, or offer a guided `PATH` fix?]
- **FR-007**: Specrew MUST provide a Unix bootstrap path (e.g. `install.sh` or equivalent) that presents a shell-native flow to install Specrew from PSGallery using PowerShell internally and then install the wrappers. The bootstrap MUST verify `pwsh` is present (clear error + install hint otherwise) and MUST NOT install PowerShell itself. [NEEDS CLARIFICATION: bootstrap delivery + sequencing — is `install.sh` a repo-committed file users `curl | sh`, a documented one-liner, or a file shipped in the module? How is the chicken-and-egg resolved (the bootstrap needs the module installed before `install-shell-wrappers` exists)?]
...
- **FR-009**: Wrapper file names MUST be generated from, or parity-validated against, the canonical Specrew command registry. The v1 registry is `Specrew.psd1` `AliasesToExport` plus the root `specrew` command; when Proposal 150's `.specrew/agent-command-manifest.json` exists, the wrapper set MUST consume or validate against that manifest. [NEEDS CLARIFICATION: for v1, generate wrappers from the registry, or commit static wrapper files and parity-validate them in CI? Proposal 153 prefers generation but allows validation where packaging constrains it — this is the load-bearing architecture choice for plan.]
