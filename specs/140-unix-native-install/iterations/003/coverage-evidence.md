...
authoritative macOS runtime is the `validate-macos` CI lane (pending push) + the T021 manual proof. Each
...
> step run on the `validate-macos` CI lane (T020), which has **not yet executed** (branch not pushed). The
> clean macOS `install.sh` auto-install + `specrew init`/`start` are the T021 **manual** proof (PENDING).

## Coverage Estimate

- Kind: qualitative
- Label: focused_regression + host-independent shell suites; macOS runtime pending CI + manual
...
| FR-012, FR-002/003/004/008 (macOS) | `wrapper-runtime.sh` on `validate-macos` (T020) + Ubuntu T015 mechanism | pending macOS CI run |
...
| SC-001/SC-003 (macOS) | `wrapper-runtime.sh` on macOS (T020) | pending macOS CI run |
...
- FR-012 + macOS halves of SC-001/SC-003: require the `validate-macos` lane to RUN green (push the branch).
