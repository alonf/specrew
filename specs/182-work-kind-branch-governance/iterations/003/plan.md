...
| ---- | ----- | ----------- | ----- | ------ | ----- | ---------------- | ------ | ----- | ------ | ------- |
| T301 | Neutralize the shared closeout-SDLC prose (G1+G2+G3): project-agnostic + forge-neutral PR/MR + publish-per-project-mechanism + review_gate opt-in | FR-019 | US2 | 3 | Spec Steward | extensions/specrew-speckit/prompts/coordinator-decision-guidance.md | done | claude | 3 | neutralized; bare mandate=0, labeled example present in all 3 sources |
| T302 | Regenerate + verify the lifecycle-prompt Rule 46/47 feature-closeout block (G4) reflects the neutralized closeout (no GitHub-only mandate) | FR-019 | US2 | 1 | Spec Steward | extensions/specrew-speckit/squad-templates/coordinator/specrew-governance.md | done | claude | 0.5 | DRIFT D-301: G4 is the assembled view of G1-G3 (no separate artifact); neutralized by T301; verified |
| T303 | D1 disposition (per DP-2 ruling): label `lifecycle-discipline.md` SDLC as a non-mandatory example OR record the own-doc exclusion | FR-019 | US2 | 1.5 | Spec Steward | docs/methodology/lifecycle-discipline.md | done | claude | 1.5 | DP-2: labeled the Repository Conventions + Post-Merge SDLC as Specrew's-own-example, not a downstream mandate |
| T304 | Route reviewer detection through the adapter (G5): Copilot becomes a GitHub-adapter opt-in suggestion, not a baked-in reviewer | FR-019 | US5 | 2.5 | Implementer | extensions/specrew-speckit/scripts/shared-governance.ps1 | done | claude | 2.5 | opt-in gate (Get-SpecrewAutomatedReviewOptIn) added; default OFF; .specify mirror synced; pr-review-integration green; PSScriptAnalyzer 0 errors |
| T305 | Tests for the G5 reviewer-routing change (adapter-mediated; the generic/non-GitHub path bakes in no Copilot) | FR-019 | US5 | 1.5 | Implementer | tests/unit/forge-neutral-reviewer.tests.ps1 | done | claude | 1.5 | 10 assertions green (opt-out default; non-GitHub never bakes in Copilot; capability-gated) |
| T306 | SC-008 scope-aware no-over-claim sweep test: downstream-governing surfaces carry no GitHub-only mandate; allowlist mirrors the inventory (host-adapter + own-infra exempt) | SC-008 | US2 | 2.5 | Reviewer | tests/integration/forge-neutralization-sweep.tests.ps1 | done | claude | 2.5 | section-aware (forge-mandate) + file-level (Specrew-publish); 44 surfaces; CAUGHT proposal-discipline.md (D-303, neutralized) |
| T307 | SC-013 verification: inventory complete; migrated surfaces carry no GitHub-only mandate; Specrew's own infra is unchanged (diff-verify own-infra untouched) | SC-013 | US2 | 1 | Reviewer | tests/integration/forge-neutralization-sweep.tests.ps1 | done | claude | 1 | own-infra/host-adapter still carry GitHub usage; allowlist inventory-backed |
| T308 | Verify Specrew's OWN closeout flow still works post-edit (the provider:github instantiation still yields the gh + beta-publish steps) | FR-019 | US2 | 1 | Reviewer | specs/182-work-kind-branch-governance/iterations/003/neutralization-inventory.md | done | claude | 1 | Specrew governance opts-in (Active reviewer); labeled example preserves Specrew's gh+PSGallery steps |
