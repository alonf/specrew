...
| --- | --- | --- | --- | --- |
| D1 | `docs/methodology/lifecycle-discipline.md` (Repository Conventions; Post-Merge SDLC table; PSGallery Publishing; Reviewer Checklist sections) | methodology-wording (doc) | Same `gh pr create` + Copilot + `Install-Module Specrew` closeout SDLC as G1–G3, plus PSGallery-publishing + reviewer-checklist Specrew references, in a downstream-referenced methodology doc (a refocus deep-source) | **DP-2 ruling = downstream-governing → labeled (T303).** A doc-level forge-neutrality note (Repository Conventions) + a "Specrew's own example" heading frame the GitHub + PSGallery specifics as Specrew-specific, NOT a downstream mandate; the content stays documented/usable for Specrew. The `Install-Module Specrew`/`Find-Module Specrew` tokens are inherently Specrew-self-references (a downstream project never installs "Specrew"), so the doc-level marker covers them (T306 file-level rule); `gh pr create` is section-labeled. |
| D2 | `docs/methodology/proposal-discipline.md` (step 11 of "Creating a New Proposal") | methodology-wording (doc) | `gh pr create` mandate for proposal PRs — a downstream-governing doc the Iter-1 inventory AND the planning sweep both missed; the T306 SC-008 sweep CAUGHT it | **CHANGE (genericized, T306):** "open a PR/MR via your forge (the provider adapter describes how)"; the `gh pr create` token removed; the now-formalized Proposal-182 reference updated. Drift D-303. **Maintainer-ratified at review boundary (keep as inheritable methodology, not Specrew-only infra; the generic PR/MR wording is correct).** In scope per "docs where they govern downstream projects". |
| D3 | `docs/methodology/README.md` (lines 9, 19) + `docs/methodology/review-instructions.md` (line 18) | methodology-wording (doc) | `PSGallery` named as a heading-descriptor of `lifecycle-discipline.md`'s Release Process Discipline section, in two downstream-governing methodology INDEX docs with NO example label. The narrow 4-token T306 sweep MISSED it (`PSGallery` was not a swept token); the post-implementation SC-008 broad-verification grep CAUGHT it (gate-coverage directional blind spot, review-signoff discipline Rule 6). | **CHANGE (neutralized by removal, T303+T306 completion):** registry-name descriptors genericized to `package-registry` / `prerelease-vs-stable` (the index points at the now-labeled section neutrally). The T306 sweep token set WIDENED to add the registry-name class (`PSGallery` / `powershellgallery` / `PowerShell Gallery`, file-level marker tier) so this class is regression-guarded; `specrew-update` allowlisted as own-infra (Section C). Drift D-304. |
...
| `extensions/specrew-speckit/squad-templates/skills/specrew-update/SKILL.md` (`PSGallery` version-available check) | Specrew's OWN update skill — `PSGallery` names where the Specrew module itself lives (a downstream project pulls Specrew from there; it is not a mandate on the downstream's OWN deliverable registry). Allowlisted in the T306 sweep when the registry-name token class was widened (D-304). |
