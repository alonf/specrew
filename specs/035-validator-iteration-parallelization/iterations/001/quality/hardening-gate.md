...
| `determinism` | `determinism` | `addressed` | `runtime-evidence` | `recorded` | Parallel and serial paths must produce identical output (modulo timing). Output must be sorted by iteration path. | `true` | Results merged in $targets order via `@($targets | ForEach-Object { ... })`; rendering pipeline unchanged. | `✅ evidence recorded` |
