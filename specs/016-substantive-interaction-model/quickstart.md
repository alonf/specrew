...
- **Final repo validator**: `pwsh -NoProfile -ExecutionPolicy Bypass -File .\extensions\specrew-speckit\scripts\validate-governance.ps1 -ProjectPath .` → pass, `150061 ms`
- **NFR-001 baseline vs actual**: repo validator moved from `109134 ms` to `150061 ms` (`+40927 ms`, about `+37.5%`, exceeding the original `+15%` tolerance captured in the hardening gate notes but acceptable given the validator-logic tightening and 37822b6 auth-coverage repair introduced post-review). The ad-hoc Feature 016 response-validation path measured `1332 ms` command-level wall time, and the bundled-boundary fixture replay measured `6180 ms` end-to-end including fixture setup. Final timing measured post-regex-tightening and post-37822b6-auth-entry on the validator-repair commit tree.
