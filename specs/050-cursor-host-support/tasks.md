...
| T002 | Implement `New-CursorLaunchInvocation` (INTERACTIVE `"<prompt>" --workspace <proj>`; `--force` only under `-AllowAll`/`-UseAutopilot`; `--trust` headless-only, unused) + unit test | FR-002, FR-009, FR-011, SC-001 | `hosts/cursor/handlers.ps1`, `tests/integration/host-cursor.tests.ps1` |
| T003 | Implement `ConvertTo-CursorFlag` (`[ValidateSet('--remote','--allow-all','--autopilot')]`: allow-all→`--force`; autopilot→no-op; remote→warn-and-drop) + unit test | FR-002 | `hosts/cursor/handlers.ps1`, `tests/integration/host-cursor.tests.ps1` |
| T004 | Implement `Test-CursorRuntimeInstalled` (detect deployed crew runtime at `.cursor/rules/*.mdc` per host contract; never throws) + unit test | FR-002 | `hosts/cursor/handlers.ps1`, `tests/integration/host-cursor.tests.ps1` |
| T005 | Implement `Get-CursorSignals` (return confirmed Cursor env-var names; document any uncertainty) + unit test | FR-002 | `hosts/cursor/handlers.ps1`, `tests/integration/host-cursor.tests.ps1` |
| T006 | Implement `Install-CursorCrewRuntime` (translate `.specrew/team/agents/*.md` → `.cursor/rules/<role>.mdc`; `-DryRun`; idempotent) + unit test | FR-002, FR-010 | `hosts/cursor/handlers.ps1`, `tests/integration/host-cursor.tests.ps1` |
...
| T010 | Verify registry auto-discovery: `Get-RegisteredHostKinds` includes `cursor`, `Test-HostManifestValid` passes, structural firewall test still green | FR-004, SC-006 | (verification; `tests/integration/host-cursor.tests.ps1`) |
...
| T015 | Add Cursor interaction-model section to `docs/user-guide.md` (rules-context vs slash commands; `--allow-all`→`--force` semantics; interactive launch) | FR-008, SC-007 | `docs/user-guide.md` |
