...
- **SPECREW-VERDICT-BOUNDARY marker**: the preferred host-neutral verdict-capture contract (an HTML comment) emitted at a boundary verdict and captured into `verdict_history`.
- **Pending verdict stop artifact**: `.specrew/runtime/pending-verdict-stop.md`, written by boundary sync whenever the mechanical cursor is ahead of the last human-authorized boundary. It is the deterministic first-render source for the exact boundary crossing, approval phrase, last-line marker, and concise approval option. It is also the deterministic fallback binding source when a transcript-capable weak host drops or mis-targets the invisible marker but the transcript contains a clear human approval for the single pending gate.
...
- **FR-015** (197-coordination; maintainer rules 2026-06-20..22): packet rendering is owed when the human must re-orient, not because the assistant response is long. Boundary/gate stops MUST render the full Rule-46 six-section re-entry packet (What I Just Did / Why I Stopped / What Needs Your Review / What Happens Next / Discussion Prompts / What I Need From You) plus the numbered verdict block and the exact `SPECREW-VERDICT-BOUNDARY` marker. Non-boundary stops after deterministic material work this turn (changed user files or new commits from the rolling-handover Stop snapshot) MUST render the five-part context packet (What I Just Did / Why I Stopped / What Needs Your Review / What Happens Next / What I Need From You), with `file:///` references and no verdict marker. Quick discussion, status, or question-answer turns with no current material signal MUST stay conversational. The design workshop's per-lens questions remain excluded, including the initial pre-boundary scaffold before `session_state`/`lens-applicability.json` exists. Enforcement EXTENDS FR-011 through the same host-neutral Stop provider and `StopBlockShape` dispatcher path; the hook enforces only bounded deterministic signals, while duration/disconnect nuance stays in refocus-core guidance.
- **FR-016** (F-185 live Codex double-stop fix): boundary sync MUST write and surface a deterministic pending-verdict stop artifact at `.specrew/runtime/pending-verdict-stop.md` whenever the mechanical cursor is ahead of `last_authorized_boundary`. The artifact and sync output MUST name the single authorizable crossing from `Get-SpecrewPendingBoundaryCrossing`, the exact human approval phrase (`approved for <to>`), the concise approval aliases (`1`, `option 1`) only for the approval option, and the exact `SPECREW-VERDICT-BOUNDARY` marker to emit as the last line. In a multi-boundary over-advance, it MUST still surface the first unpaid crossing (for example `intake -> specify`), never the later working boundary (for example `specify -> clarify` or `clarify -> plan`). When no verdict is pending, sync MUST remove any stale artifact. Verdict capture MUST run on prompt-entry hooks (`UserPromptSubmit` where available, `PreInvocation` for Antigravity) as the immediate path and on Stop as the backstop, prefer the marker when present and matching, and when the marker is missing or wrong on a transcript-capable host, MAY bind a clear human approval to the pending-verdict artifact only for the single pending crossing, only from real user text, and only when the approval is explicit (`approved for <to>`) or a concise `1` / `option 1` response to a packet whose option 1 is visibly approval. Vague replies (`yes`, `ok`, `continue`), rejection/discussion options, `option 2`, contradictory named boundaries, and no-pending-state cases MUST NOT authorize.
...
- **SC-012** (FR-016): automated coverage proves `sync-boundary-state.ps1` writes the pending-verdict stop artifact for the first boundary, preserves the first unpaid crossing during a multi-boundary over-advance, surfaces `approved for specify` rather than `approved for clarify` when `intake -> specify` is still pending, and removes a stale artifact when no pending verdict remains.
- **SC-013** (FR-016): automated coverage proves transcript verdict capture parses Antigravity `MODEL` / `USER_EXPLICIT` turns, captures markerless packet approvals through pending-verdict fallback for `1` and `option 1`, captures the current `UserPromptSubmit` / `PreInvocation` text before the transcript has appended it, records the fallback evidence source distinctly, and rejects markerless fallback when there is no pending verdict, a contradictory named boundary, or `option 2` / rejection-style response.
