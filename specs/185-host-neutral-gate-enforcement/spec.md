...
- **FR-015** (197-coordination; maintainer rule 2026-06-20): the full Rule-46 six-section re-entry packet (What I Just Did / Why I Stopped / What Needs Your Review / What Happens Next / Discussion Prompts / What I Need From You) MUST render at EVERY stop where the agent yields the turn to the human — not only at formal verdict/lifecycle boundaries (a within-phase task checkpoint or a "proceed?" hand-off is a stop and owes the packet); the design workshop's per-lens questions are the ONLY exclusion. Form: full sectioned markdown + bare `file:///` URLs + a numbered verdict block (never a compressed "1. Why:… 2. What:…" list); Claude via the gate-stop skill, other hosts render directly. The rule MUST be carried in the refocus CORE (`general.md`) on every host — it is encoded nowhere today, the drift that let it be skipped. Enforcement EXTENDS FR-011: the Stop-hook authority checks the every-stop marker/packet discipline (today FR-011 catches only an advance past `last_authorized_boundary` with no verdict-stop marker → halt; extend the principle to every stop). The FR-011 Stop-hook is built as a host-neutral MULTI-CONSUMER seam (consumers register "call me at stop-kind X"): 185's own consumers are the verdict/gate enforcement + this every-stop packet check; F-197's always-on co-review registers its reviewer dispatcher on the same seat — one Stop-hook seat, multiple consumers, not parallel hooks. Evidence: a general.md assertion the rule is encoded + a real-host dogfood that a non-boundary stop shows the packet.
...
- **SC-011** (FR-015): the every-stop re-entry-packet rule is encoded in the refocus core (`general.md` rule 9), extends to within-phase checkpoints, keeps the verdict MARKER boundary-scoped, and preserves the clarify/workshop normal path — automated test (`every-stop-packet.tests.ps1`) — plus a real-host dogfood that a non-boundary "proceed?" stop renders the full six-section packet (manual). The FR-011 Stop-hook enforcement of this rule rides the existing dispatcher as the `conformance` provider.
