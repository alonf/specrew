...
| & '.\\tests\\integration\\refocus-deploy.tests.ps1' | pass | 59 | 0 | ~00:45 | 0 | T014+T017 deliverable — 19→59: codex/copilot/cursor writers + overlay merge round-trip/fail-safe + wiring content/ordering/parse asserts (incl. the rework's init-anchor ordering guard) |
...
| & '.\\tests\\integration\\bootstrap-to-iteration.ps1' | pass | all | 0 | ~04:00 | 0 | T017 consumer-side demonstration, run TWICE: first run exposed the init-anchor defect (no claude deploy on greenfield); post-rework re-run's action log shows `refocus-hooks` for all four hosts + the scratch project's `.claude/settings.local.json` carrying our SessionStart entry |
