...
| FR-003 | Scope→digest, trigger→scope, per-trigger token budgets, and the provider registry MUST live in a data-driven catalog (`refocus-scopes.yml`) carrying a required `schema_version`; evolution is additive-only; version mismatch fails open with `CATALOG_SCHEMA` WARN. | — |
