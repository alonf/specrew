# Iteration Plan: 001 — engine, channels, dispatcher, breaker, Claude binding
...
| T006 | SpecrewHookDispatcher + dormant gate path + stdin test suite | FR-008, FR-012, FR-020 | US3 | 2.5 | Implementer | scripts/internal/specrew-hook-dispatcher.ps1, tests/integration/refocus-dispatcher.tests.ps1 | done | claude | 2.5 | pass |
| T007 | RefocusProvider routing + B3 state-diff | FR-009 | US3 | 1.5 | Implementer | scripts/internal/specrew-hook-dispatcher.ps1 | done | claude | 1.5 | pass |
| T008 | RuntimeSessionState + journal + pruning | FR-010 | US3 | 1.5 | Implementer | scripts/internal/specrew-hook-dispatcher.ps1 | done | claude | 1.5 | pass |
| T009 | Circuit breaker + --status + --reset-breaker | FR-011 | US4 | 1.5 | Implementer | scripts/internal/specrew-hook-dispatcher.ps1, scripts/internal/refocus.ps1 | done | claude | 1.5 | pass |
| T010 | Claude binding + merge-aware hook deploy + opt-out memory | FR-013, FR-014, FR-020 | US3 | 2.0 | Implementer | hosts/claude/**, tests/integration/refocus-deploy.tests.ps1 | done | claude | 2.0 | pass |
| T011 | Skill manifest + per-host deploy + FileList | FR-015, FR-018 | US1 | 1.5 | Implementer | extensions/specrew-speckit/squad-templates/skills/specrew-refocus/**, Specrew.psd1 | done | claude | 1.5 | pass |
| T012 | Compaction points (--compact-instructions) + coordinator advisory | FR-016, FR-017 | US5 | 1.0 | Implementer | scripts/internal/refocus.ps1, extensions/specrew-speckit/squad-templates/coordinator/specrew-governance.md | done | claude | 1.0 | pass |
...
| ----- | ---------------- | ----- |
| Planning | 0 | Completed in the lifecycle ceremony (workshop + design-analysis + plan + tasks boundaries) |
| Discovery/Spikes | 0 | Iteration-001 surfaces are documented (Claude); host research is iteration-002 (T013) |
| Implementation | 18.5 | Sum of T001-T012 per the task table |
| Review | 0 | Review-signoff effort tracked at the boundary, not as task SP |
| Rework | 0 | Needs-rework buffer handled by the 1.5 SP headroom under the 20 SP cap |
...
- User stories represented in current scope: US1 (manual refocus), US2 (boundary-cross all hosts), US3 (hook triggers), US4 (operator safety), US5 (managed compaction points)
- Capacity check: 18.5/20 story_points planned — under the cap with 1.5 SP rework headroom; no deferrals required.
- Traceability: task-to-FR/SC map verified in ../../tasks.md (after-tasks check, all 20 FRs + 10 SCs covered).
