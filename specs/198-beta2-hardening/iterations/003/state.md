...
- T018 completion record (carried from the pre-repair Last Completed Task header line): T018 (universal language/framework-NEUTRAL recorded-run evidence runner `Invoke-ContinuousCoReviewRecordedRun` — direct command-execution facts + OPTIONAL run-produced schema-valid SpecrewTestResult; caller counts forbidden; fail-loud; evidence-only; 10/10 focused + full 19-suite registry green; T013/T014/T015/T016/T017/T020 also complete)
- T019 in-progress record (carried from the pre-repair In Progress header line): T019 — the CHARACTERIZATION/CONTRACT slice (steps 1–5) is DONE and green through TWO correction passes: the needs-rework 6 corrections, THEN the FINAL pre-step-6 corrections (same-digest authority = atomically-acquired LEASE/generation not max-run_id + clean-never-erases-blocking; new FR-045 required-review-not-started + human-dispositioned states; canonicalized lineage id — 21/21 focused + registry). Per the maintainer's 2026-07-13 directive ("proceed directly to step 6 if green; no additional contract-review stop"), step 6 (implement against the contracts — runtime wiring) is UNDERWAY, incrementally with paired runtime tests. **Piece 1 DONE**: the registry-key-drift fix (the DRIFT-002 root cause) — the navigator's stale-verdict downgrade + blackboard stamp read reviewed_tree_id, a key the live fire path never writes, so the digest-match-before-blocking was skipped and stale blocks recurred; now ONE resolver (Get-ContinuousCoReviewNavigatorRegistryTreeId) reads the reviewed_digest_tree_id the fire path writes, with a paired stale-downgrade test (previously uncovered). The resolver was then HARDENED (maintainer 2026-07-13): prefer the EXPLICIT ids (reviewed_digest_tree_id -> reviewed_tree_id), tree_id legacy-only, and FAIL CLOSED with a named reviewed-tree-identity-conflict when two explicit ids disagree (no block/promote/stamp using either value) — paired agree/conflict/legacy resolver tests + a reap-level conflict test. Navigator suite 6/6. **Piece 2 DONE (2a+2b+2c)** — the per-lineage review LEASE (maintainer design 2026-07-13): 2a the lease module (immutable reviewed-tree-digest generation; atomic CreateNew winner; duplicate-same-generation = no acquire; newer-tree queueing; owner-only release; dead-owner reclaim with PID + process-start identity; SHA-256 lineage filename; 4-condition promotion authority — 10/10 covering all 9 required cases incl. a real concurrent single-winner race); 2b the spawn is lease-gated in co-review-service (acquire before the reviewer spawn; a failed acquire SUPPRESSES the spawn and consumes no spend/round; supervisor PID stamped; lease released on spawn failure); 2c the reap enforces the 4-condition authority (a non-owner/stale-generation completion is advisory only, never blocks/promotes) and releases the owner's lease on retirement (feeding the pending tree forward). Both contract modules wired into _load.ps1; the lease + service + navigator suites are in the F-198 registry (22->23 suites). **Piece 3 DONE** — the navigator FIRE decision (worktree-navigator.ps1) now CONSUMES the per-lineage lease as the SINGLE in-flight dedup source (not a second competing mechanism): a lineage already under review by a live owner is SUPPRESSED at the lease acquire before any reviewer spawns (the DRIFT-002 collision class a manual --live + navigator race that last_fired_tree_id missed, since other drivers never set it); a suppressed acquire records deduped-by-lease and does NOT advance last_fired_tree_id (else a queued newer tree would be treated as already-fired and never reviewed). last_fired_tree_id stays purely the changed-tree trigger. Also fixed a piece-2b latent bug (the fire recorded ANY Start-...ServiceRun result as 'fired'). Navigator suite 9/9. **NEXT**: pieces 4–7 — the FR-045 Stop-routing packet gate (gate the verdict packet on the Stop-routing contract), evidence-injection runs+embedded validation, findings-result tree/baseline stamping, and the retention runtime (tested BEFORE touching residue). Runtime residue stays untouched. Also flagged (release-blocker interrupt): released 0.40.0 Squad-runtime deploy packaging fixed (197 contracts + step-6 modules + tracker-honesty added to FileList; packaged-artifact integration test)
...
- FR-055 RECORDED (2026-07-14, maintainer-directed): non-boundary Stop-packet classification hardening —
  turn-delta baseline, long-turn lane, PostToolUse pre-arrangement nudge, six regression fixtures. See
  DRIFT-198-I003-008 in drift-log.md (includes the Case-5 already-red-at-HEAD reconciliation) and the
  iteration-005 Seventh-round record for the same verdict's f2/f3-residual machinery fixes.
- STATE-NARRATIVE DESTRUCTION ROOT CAUSE FOUND + FIXED (2026-07-14, DRIFT-198-I003-009): the recurring
  committed-state truncations (this file at 2026-07-14T15:29Z; the iteration-003 reset repaired in 47106751)
  were caused by Update-IterationStateFromTaskProgress replacing the ENTIRE Execution Summary section with
  its 3 generated bullets on every task-progress sync. The digest now refreshes a marker-bounded managed
  block and hand-authored narrative is preserved (scripts/internal/task-progress.ps1;
  tests/unit/task-progress-managed-summary.tests.ps1, in the F-198 registry). This file is the repaired
  restore of the committed rich record with the canonical header + managed digest merged in.
