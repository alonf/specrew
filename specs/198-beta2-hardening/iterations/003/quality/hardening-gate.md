...
| --- | --- | --- | --- |
| `condition-a-human-authorization` | `open-for-verdict` | NOT met at authoring time and must not claim to be: the `tasks -> before-implement` verdict has not been captured. This is a planning-time gate; condition-a stays `open-for-verdict` until the genuine before-implement authorization is hook-captured. The design approval (Option B, maintainer-typed) and the tasks approval explicitly do NOT authorize implementation (maintainer instruction at the planning approval: "This approval authorizes planning only"). Corrects a latent false-green pattern (iterations 001/002 marked this `met` at authoring; maintainer send-back 2026-07-11). | Implementation starts only when this condition flips to met on the explicit, captured before-implement verdict — never on gate authoring. |
