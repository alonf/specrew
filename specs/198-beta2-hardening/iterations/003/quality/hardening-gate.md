...
Recorded at the retroactive iteration-closeout on 2026-07-27. This section
POINTS at where each concern's runtime evidence actually lives. It does NOT
claim Iteration 003 produced it.

Every concern keeps `Evidence Basis: planning-time-analysis`, which is the
truthful record of what 003 did. Runtime Evidence Status is `not-needed`
**from 003** — not because the controls went unproven, but because 003's
residual scope moved to Iteration 007 under the authorized reconciliation at
file:///C:/Dev/specrew-beta2-hardening/specs/198-beta2-hardening/iterations/007/iteration-003-reconciliation.md,
and the runtime proof was produced there. Flipping these rows to
`runtime-evidence` / `recorded` would assert 003 recorded proof it never
recorded, which is the false-green pattern this gate exists to prevent.

| Concern | Controls delivered in 003 | Controls delivered in 007 | Where the runtime evidence lives |
| --- | --- | --- | --- |
| `security-surface` | Reviewer containment (T013–T017) | Capture integrity (T030–T033), T034b | file:///C:/Dev/specrew-beta2-hardening/specs/198-beta2-hardening/iterations/003/coverage-evidence.md and file:///C:/Dev/specrew-beta2-hardening/specs/198-beta2-hardening/iterations/007/review.md |
| `error-handling-expectations` | Violation records, rejected caller numbers, halt text, strict design-context fail | Ledger/correction fail-closed paths (T033) | file:///C:/Dev/specrew-beta2-hardening/specs/198-beta2-hardening/iterations/003/drift-log.md and file:///C:/Dev/specrew-beta2-hardening/specs/198-beta2-hardening/iterations/007/review.md |
| `retry-idempotency-requirements` | Worktree materialization, re-entrant sampling | Append-only correction door no-op (T033); T019 in-flight dedup superseded by iteration 006 identity | file:///C:/Dev/specrew-beta2-hardening/specs/198-beta2-hardening/iterations/007/review.md |
| `test-integrity-targets` | Fixture hermeticity, recorded-run wrapper (T018) | Fabrication fixtures (T032) and the fallback re-enable decision | file:///C:/Dev/specrew-beta2-hardening/specs/198-beta2-hardening/iterations/007/review.md |
| `operational-resilience-concerns` | Temp worktrees, ONE machinery list, T034a seam inspection | T034b at-landing checkpoint | file:///C:/Dev/specrew-beta2-hardening/specs/198-beta2-hardening/iterations/007/review.md |

The `condition-d-capacity-discipline` defer order recorded above was honoured
in the outcome: nothing in T013–T017 or T020 was dropped, and the deferred set
is exactly the capture-integrity class the condition named as deferrable.

## Notes

- Planning-time gate. The iteration-001/002 precedent flips per-concern
  Runtime Evidence Status to `recorded` with `runtime-evidence` basis at
  iteration close; 003 closes on the follow-through shape above instead,
  because its residual scope was delivered by a later iteration. The gate
  schema has no value meaning "runtime evidence recorded in the successor
  iteration" — recorded as a gap in
