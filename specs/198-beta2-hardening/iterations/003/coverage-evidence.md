...
| T017 (FR-012/SC-004) | `reviewed-state-digest.Tests.ps1` — ONE machinery source (Get-ContinuousCoReviewMachineryPaths) consumed by BOTH the digest strip AND the worktree strip: host-mirror machinery (.claude/skills, .github/agents) is EXCLUDED from the digest identity BY CONSTRUCTION (same source as the worktree strip — cannot drift); `.github/workflows` + ordinary source STAY IN (reviewer-can-still-see-it regression); a machinery-only change does NOT flip the digest while a SOURCE change DOES (no false-allow) — plus the pre-existing digest determinism/gitignored-source/secret-out/exec-bit tests | pass | 15 | 0 | ~25s | 0 |
| T018 (FR-014/FR-015 amended — universal recorded-run runner) | `recorded-run.Tests.ps1` — LANGUAGE/FRAMEWORK-NEUTRAL runner (`Invoke-ContinuousCoReviewRecordedRun`): generic SUCCESS → `command_succeeded` + counts UNAVAILABLE (exit 0 ≠ "all tests passed"); generic FAILURE → real exit code, NO console parsing; TIMEOUT + process-tree kill (bounded ~2s, not the full 30s); valid run-produced `SpecrewTestResult` → counts verbatim; REQUIRED-but-missing/malformed → FAIL LOUD (never a richer claim); caller counts structurally FORBIDDEN (no count parameter exists); STALE pre-existing result REJECTED (deleted before the run); EXACT-DIGEST binding (a source change re-keys the record); BOUNDED/redacted large stdout (tail + sha256, never the full dump); a PESTER ADAPTER emits the universal contract with NO Pester logic in core (adapter is downstream). In-session runner-reported (this tree); re-record against the exact reviewed digest for injected standing. | pass | 10 | 0 | ~20s | 0 |
...
| `& ./tests/f198-regression-suite.ps1` | pass | 18 | 0 | ~100s | 0 | Whole-feature honesty gate. Last recorded meta-run: **18/0 at a PRIOR digest**. The registry now REGISTERS **19 suites** — the T018 `recorded-run.Tests.ps1` suite was added — so a **full re-run at the CURRENT digest is PENDING** (18/0 predates the T018 addition; this row is NOT re-claimed as 19/19 without running it). Enumerated suites: ratchet, spend allowance, containment, origin hygiene, bounded-verification helper + reviewer-integrity, tracker honesty, verdict capture, budget, signoff gate, shared-engine, digest/exec-bit, reviewer hook-suppression, file-primary result, **recorded-run (T018)**. When re-run, runner-reported for the recorded digest and written to the digest-keyed machine record; evidence standing follows the exact-digest-injection note above (a partial-injection review is DRIFT-198-I003-002 behavior, not proof the run did not occur). |
...
| FR-014, FR-015 (AMENDED 2026-07-13 — universal recorded-run runner: framework-neutral execution facts + OPTIONAL run-produced SpecrewTestResult; caller counts forbidden; fail-loud; refocus duty floor) | tests/continuous-co-review/unit/recorded-run.Tests.ps1 |
| NFR-007 (CI enforcement) | tests/f198-regression-suite.ps1 (blocking CI step; now registers the recorded-run suite) |
