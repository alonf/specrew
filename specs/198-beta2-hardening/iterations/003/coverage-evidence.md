...
| codex-reviewer hardening (empty-exit0) | `reviewer-hook-suppression.Tests.ps1` (reviewer spawn passes SPECREW_REFOCUS_DISABLE=1; dispatcher exits before governance when it inherits it) + `reviewer-file-primary-result.Tests.ps1` (a clean-exit, current-run, fully-contract-validated `.review/findings.jsonl` is a FULL file-primary result — no wasteful T108 retry, no 'partial' mislabel; FAIL-CLOSED on malformed/truncated/stale/mismatched-run/absent/empty; normal stdout-primary hosts unchanged) | pass | 13 | 0 | ~5s | 0 |
