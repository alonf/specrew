...
| `condition-d-capacity-discipline` | `met-with-explicit-overcommit` | 26.5/26 SP includes T068/T069/T070, supplier/injection, distribution, all seven observed T066 correction classes, deterministic proof, review, release, and dogfood. The +17% forecast over 7.75 SP still-open work is 27.82/26. | The standing progress authorization covers the 0.5 SP evidence-driven correction overcommit; add no optional scope and keep the 1.82 SP calibration risk visible. |
...
| `condition-g-provider-authority` | `met` | Task/before-implement authorization granted zero provider invocations. T066 attempts 01, 02, 05, 06, 07, and 08 each spent exactly one separately recorded Claude slot. Attempts 03 and 04 stopped during controller verification with zero invocation/spend and released reservations. The maintainer later granted standing bounded correction/review authority while progress is demonstrable. | Every provider action still requires a new run ID, one immutable slot fact, changed evidence, no hidden retry, and the non-convergence stop rule. Snapshot-integrity failure in attempts 07/08 means one further recurrence stops for replan. |
