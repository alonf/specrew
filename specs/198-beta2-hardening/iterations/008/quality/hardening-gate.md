...
| `condition-d-capacity-discipline` | `met` | 25.5/26 SP includes T068/T069/T070, supplier/injection, distribution, all five observed T066 correction classes, deterministic proof, review, release, and dogfood. The +17% forecast over 6.75 SP still-open work is 26.65/26. | Preserve the remaining 0.5 SP nominal headroom, add no scope, and keep the 0.65 SP conservative calibration risk visible. |
...
campaign owns final exact-candidate full proof. A support-lifecycle finding in the next provider result reaches the third
consecutive support-class round and must stop for replan.
