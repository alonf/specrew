...
| `exact-digest-evidence-injection` | `evidence-integrity` | `addressed` | `runtime-evidence` | `recorded` | T064 freezes the selected-plan bytes with the external target, binds their hash into currentness, runs T018 before provider launch, requires one exact digest + command-ID join per declaration, and injects one bounded evidence file. Seven production-path fixtures prove ordered mixed success, zero-spend missing/invalid and mutation refusal, failed-command non-approval, stale/unjoinable refusal, plan drift, and unchanged origin HEAD/status. | `true` | Useful evidence for another tree must remain visible but cannot approve the current tree. | `—` |
