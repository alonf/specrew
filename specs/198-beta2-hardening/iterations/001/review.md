...
- Independent-review catch (run 20260710T181409652, copilot - its first round as a wired reviewer, 82s): the annotation checker extracted the human's reason only to null-check it and dropped it - the [annotated] output never surfaced the audit reason and Test 3 asserted only the literal token (a reason-dropping regression would have passed). Fixed: the checker returns the reason, records carry AnnotationReason, output prints it per hit, Test 3 asserts the actual reason text (21/21 green): fixed-now.
