...
| ---- | ----- | ----------- | ----- | ------ | ----- | ---------------- | ------ | ----- | ------ | ------- |
| T080 | Link-state fixtures for the volume-differential harness, proven RED first | FR-008 | Containment coverage | 6.0 | Implementer | tests/continuous-co-review/unit/path-identity-volume-differential.Tests.ps1 | done | Implementer | 6.0 | Status corrected 2026-08-01 — it read `planned` while the fixtures and their three-volume evidence were committed (DRIFT-198-I010-005). Link-state fixtures delivered; their measurement is what disproved DRIFT-198-I009-042 |
...
| T082 | DRIFT-198-I009-041 — authority-store containment against reparse points | FR-008 | Evidence-chain integrity | 5.0 | Implementer | scripts/internal/continuous-co-review/review-authority-store.ps1, tests/continuous-co-review/unit/review-authority-store.Tests.ps1 | needs-rework | Implementer | 5.0 | **Status corrected 2026-08-01 from `done` — the correction is PARTIAL and -041 is NOT delivered.** Path RESOLUTION is contained (RED 3/15 pre-fix, GREEN 15/15 post-fix, mutation-gated by T081) but ENUMERATION-derived paths bypass the choke point at four sites — DRIFT-198-I010-004, blocking. The `done` status rested on a coverage claim never verified |
...
| T085 | Integrated verification and capped certification | FR-008, FR-046 | Release confidence | 2.5 | Reviewer | tests/**, specs/198-beta2-hardening/iterations/010/** | deferred | Reviewer | 2.5 | **Terminal as DEFERRED, not done — verification PASSED, certification did NOT.** Registry 88/88, bootstrap, lint, three-volume matrix all green against the exact digest; `run-f198-i010-64878edb-certify` (digest `0f53945e`) returned `can_approve_current: false` with one BLOCKING containment finding of a class already corrected here (DRIFT-198-I010-004) plus one major records finding (-005). **Round 1 of a 3-round cap — the termination rule fired first**, so the remaining rounds are deliberately unspent |
...
| --- | --- | --- |
| 1 | Authority-store containment unproven against reparse points | **NOT REMOVED — partially corrected.** T082 contained path RESOLUTION; ENUMERATION-derived paths bypass the choke point at four sites (DRIFT-198-I010-004, blocking). The limitation must be REVISED to describe the partial state, not struck |
