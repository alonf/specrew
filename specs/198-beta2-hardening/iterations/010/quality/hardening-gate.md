...
| `retry-idempotency-requirements` | `retry-idempotency` | `not-applicable` | `not-applicable` | `not-needed` | No new retry, lease, or resumable operation is introduced. | `false` | This iteration adds no retry surface. The authority-store writes T082 hardens are already append-only with existing idempotency guarantees (immutable facts, CreateNew semantics), and T082 changes only the containment check that precedes them — it does not alter write or retry semantics. T080/T081/T084 are test-only. | `—` |
