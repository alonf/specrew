...
| `retry-idempotency-requirements` | `retry-idempotency` | `addressed` | `runtime-evidence` | `recorded` | Human-only slot grants; preflight before spend; one run ID per invocation; no hidden retry; five slots are best-case floor; every T061 correction rerun stops for a new grant. | `true` | T061 has eleven unique attempts, nine provider invocations/spends, and no hidden retry; the finalization ledger must add runs 11 and 12. | `auth-b3798462` |
| `test-integrity-targets` | `test-integrity` | `addressed` | `runtime-evidence` | `recorded` | Paired fixtures, all-adapter matrix on three OSes, real process-tree tests, one paid smoke per harness, at least one live run per OS, unavailable remains unproven, final exact-digest signoff. | `true` | Prior 57-suite/three-OS/T060 proof remains recorded; run 11 found DRIFT-027, so a fresh full gate and clean T061 result remain mandatory. | `auth-b3798462` |
...
| `currentness-partial-and-retro-usefulness` | `integrity` | `addressed` | `runtime-evidence` | `recorded` | Exact target digest/currentness; moved/partial findings advisory and visible; only complete valid current pass approves; retro consumes validated controller facts with provenance. | `true` | Run 11 is current valid one-finding evidence for its exact digest; run 10 and all earlier outcomes remain immutable retrospective inputs but cannot approve the correction tree. | `auth-b3798462` |
...
| `performance-and-observability` | `performance` | `addressed` | `runtime-evidence` | `recorded` | Cheap preflight, shared-object target, bounded prompt, stages/heartbeat/timing, valid-checkpoint finding counts, safe optional usage, explicit unavailable metrics; stability/integrity remain P0. | `false` | Attempt durations, progress, failures, findings, and nine T061 spends are retained without weakening integrity. | `auth-b3798462` |
...
- Runtime evidence for T030–T034b and T051–T060 is verified. T060 has live file-primary/native-containment evidence for Codex/macOS, Cursor/Windows, Antigravity/Windows, and Copilot/Linux; only the pinned clean runs approve their own snapshots, and Cursor is explicitly not claimed clean after free-credit exhaustion. The prior disabled barrier, campaign activation, 57-suite registry, scoped governance, hosted CI, and exact-commit preflight remain recorded. T061 run 11 is complete/current/valid with verified containment/termination but one blocking finding, so T061 and review-signoff remain open until the correction passes fresh full gates and independent review. T034b/T059 deterministic compatibility does not substitute for live evidence.
- DRIFT-198-I007-026 supplies the maintainer-approved small controller-owned finalization envelope. T061 run 11 then exposed DRIFT-198-I007-027: the parameterless production gate failed to carry resolved feature/iteration identity into that validator. The scoped backfill retains the post-resolution mandatory check; paired real-path publication/rendering plus unresolved-scope rejection pass within the focused 41/41 group, all 57 process-isolated suites pass in 693.2 seconds, and scoped/gap governance pass with historical warnings only. Commit/hosted CI and the single freshly authorized independent run remain. DRIFT-198-I007-025 remains an explicitly deferred stop/capture-mechanism follow-up. The command-plan dependency is a separate feature-level slice, not a hidden deferral from this iteration.
