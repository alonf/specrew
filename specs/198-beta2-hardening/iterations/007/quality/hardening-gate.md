...
**Post-Implementation Verification**: in progress — the DRIFT-198-I007-028 correction passes 78/78 focused tests, all 57 F-198 suites in 707.8 seconds, scoped governance, and gap governance; commit/hosted CI and one fresh independent review remain
...
| `retry-idempotency-requirements` | `retry-idempotency` | `addressed` | `runtime-evidence` | `recorded` | Human-only slot grants; preflight before spend; one run ID per invocation; no hidden retry; five slots are best-case floor; every T061 correction rerun is separately identified and authorized. | `true` | T061 has twelve unique attempts, ten provider invocations/spends, and no hidden retry; the finalization ledger must add runs 11 onward. | `auth-b3798462` |
| `test-integrity-targets` | `test-integrity` | `addressed` | `runtime-evidence` | `recorded` | Paired fixtures, all-adapter matrix on three OSes, real process-tree tests, one paid smoke per harness, at least one live run per OS, unavailable remains unproven, final exact-digest signoff. | `true` | Prior three-OS/T060 proof remains recorded; run 12 found DRIFT-028 and its correction passes the full deterministic gate, while a clean current T061 result remains mandatory. | `auth-b3798462` |
...
| `currentness-partial-and-retro-usefulness` | `integrity` | `addressed` | `runtime-evidence` | `recorded` | Exact target digest/currentness; moved/partial findings advisory and visible; only complete valid current pass approves; retro consumes validated controller facts with provenance. | `true` | Run 12 is complete/current/valid three-finding evidence for its exact digest; run 10 and all earlier outcomes remain immutable retrospective inputs but cannot approve the correction tree. | `auth-b3798462` |
...
| `performance-and-observability` | `performance` | `addressed` | `runtime-evidence` | `recorded` | Cheap preflight, shared-object target, bounded prompt, stages/heartbeat/timing, valid-checkpoint finding counts, safe optional usage, explicit unavailable metrics; stability/integrity remain P0. | `false` | Attempt durations, progress, failures, findings, and ten T061 spends are retained without weakening integrity. | `auth-b3798462` |
...
- Runtime evidence for T030–T034b and T051–T060 is verified. T060 has live file-primary/native-containment evidence for Codex/macOS, Cursor/Windows, Antigravity/Windows, and Copilot/Linux; only the pinned clean runs approve their own snapshots, and Cursor is explicitly not claimed clean after free-credit exhaustion. The prior disabled barrier, campaign activation, 57-suite registry, scoped governance, hosted CI, and exact-commit preflight remain recorded. T061 run 12 is complete/current/valid with verified containment/termination but three findings, so T061 and review-signoff remain open until DRIFT-028 passes committed hosted gates and independent review. T034b/T059 deterministic compatibility does not substitute for live evidence.
- DRIFT-198-I007-026 supplies the maintainer-approved small controller-owned finalization envelope. Run 11 exposed and run 12 independently confirmed DRIFT-027's production identity backfill. Run 12 then exposed DRIFT-028's shared-identity, explicit-scope diagnostic, and concurrent-publication edges. One shared safe scope predicate, the exact pre-validation/publication guard, and a full re-validation of the immutable `CreateNew` winner now pass 78/78 focused tests, all 57 process-isolated suites in 707.8 seconds, and scoped/gap governance with historical warnings only. Commit/hosted CI and a fresh independent run remain. DRIFT-198-I007-025 remains an explicitly deferred stop/capture-mechanism follow-up. The command-plan dependency is a separate feature-level slice, not a hidden deferral from this iteration.
