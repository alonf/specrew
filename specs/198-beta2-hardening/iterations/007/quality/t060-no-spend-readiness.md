...
CI run `29601394317` and the exact-commit preflight passed for phase-scope commit `ac731737fc6be92ea9248019c429bdc8e66eaccc`, digest `a30f1aa1511c8c3c9c9cbf148b9658ff973ae4d3`. Run `run-t060-antigravity-windows-ac731737-03`, authorization `human-grant-t060-antigravity-windows-20260717-slot-3`, invoked exactly once and returned after 450343 ms under verified containment/termination. Strict ingress rejected its two-finding candidate because both `location` values were not strings. The immutable controller result is invalid-output/incomplete with zero validated findings; the raw candidate is neither salvaged nor accepted.

`DRIFT-198-I007-009` makes the existing closed schema explicit in the shared prompt: an optional location is one plain JSON string, never an object/array/number/boolean. Template validation requires the type rule for all five adapters. The strict malformed-output matrix adds object and array fixtures; the valid string-location pair remains green. The shared contract plus custom-prompt orchestration fixture pass 40/40, and all 56 F-198 suites pass in 609.7 seconds.

Commit `ba639e84589e3a89fc8440616e8a1112b6ea4c51`, CI run `29604077312`, and exact-commit no-spend preflight passed. Run `run-t060-antigravity-windows-ba639e84-04`, authorization `human-grant-t060-antigravity-windows-20260717-slot-4`, invoked exactly once and completed in 790453 ms with valid/current/contained evidence. It used a schema-valid string location and did not repeat the run-03 defect. Its one minor finding established that the stale host-neutral governance guard rejected the approved host adapters/support boundaries and was absent from the explicit 56-suite registry.

`DRIFT-198-I007-010` retains the host-neutral policy/orchestration invariant through a closed named allowlist for the catalog, adapters, support, and containment boundaries; adds Cursor to the forbidden-token set; and registers the guard as suite 57. The focused guard passes 3/3, all 57 F-198 suites pass in 605.4 seconds, and scoped governance passes. Commit `f39178e616f58df9badbdcf09f62282ec843153c`, CI run `29606585387`, and exact-commit preflight passed at digest `d3314ee2566887e5148e02ca33c04128b87d7bdf` with Antigravity `1.1.3` and Job Object readiness.
...
| Claude / Windows | T061 run 10 is complete/pass/current/valid with zero findings and verified Job Object containment/termination | live-proven clean at commit `fc1054b5` / digest `5fc6318a300afc654bb09d986d82c8c925506ed3` |

Support here means that the real harness, file-primary contract, and native containment path executed successfully. It does not promote an older digest as approval of a newer tree. Final T061 run 10 is the only result that approves the reviewed final campaign snapshot.

## Current Decision

Preserve the clean Mac result, every immutable Cursor attempt, all five Antigravity runs, all four Copilot runs, and all ten T061 attempts. Cursor clean proof is explicitly unavailable until free credit resets; do not use paid on-demand credit or retry an unchanged exhausted account. The four T060 harness/OS paths are truthfully proved at their pinned digests. The one-way transition is persisted through disabled commit `233de034ddd13382dc1f58ba7323ff7b89a1c0b0` and campaign commit `fc52b6d1d9c6a4c17489f87dbfd9e4534300c0a7`; the DRIFT-013 follow-up passes focused 13/13, all 57 suites, scoped governance, and hosted three-OS CI `29612794643`. T061 run 10 completes independent exact-digest signoff; `review.md` records the complete ten-attempt/eight-spend ledger. No hosted-macOS provider workflow or GitHub Actions credential secret is part of T060.
