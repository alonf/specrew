...
| 2 | T060 | Windows | Antigravity | attempt 01 produced corrected defects; run 02 exposed phase scope; run 03 exposed ambiguous `location` typing and was strictly rejected | one prompt-contract verification attempt remains under minimum-budget standing authority |
...
Preserve the clean Mac result, every immutable Cursor attempt, and all three Antigravity runs. Commit/push the deterministically green shared prompt correction, require green CI and a fresh exact-commit no-spend preflight, then use one new-ID Antigravity verification run. Cursor clean proof is explicitly deferred until free credit resets; do not use paid on-demand credit or retry an unchanged exhausted account. Copilot/Linux and T061 Claude remain serialized after Antigravity. No hosted-macOS provider workflow or GitHub Actions credential secret is part of T060.
