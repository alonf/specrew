...
Preserve the clean Mac result and every immutable Cursor attempt. Push the run-06 correction, require green CI and a fresh exact-commit no-spend preflight, then spend only the already-authorized clean Cursor rerun with a new run ID and authorization reference. A clean run advances to the serialized Antigravity/Windows, Copilot/Linux, and T061 Claude proofs; any non-clean result stops. No hosted-macOS provider workflow or GitHub Actions credential secret is part of T060.
