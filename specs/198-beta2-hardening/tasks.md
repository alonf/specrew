...
- [ ] T020 [owner: Implementer] [sp: 1.0] **Review-loop spend allowance UX** — consumer-legible allowance-halt text (spend-guard explanation, N-of-M rounds, exact human-typed reset command, resolved-vs-open state computed from the disposition trail); every round counts (clarify 2026-07-10 supersedes the no-increment design); an input-never-materialized run is an infra failure — loud, durably recorded, never a spend-consuming round (second Devin field report, changes.diff materialization failure); message-content test asserts zero Specrew-internal identifiers in the halt (Trace: FR-018, FR-019, SC-007, NFR-007; owns: ceiling governor + halt text)
