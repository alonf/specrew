...
- [x] T016 [owner: Implementer] [sp: 1.0] **Containment detector** — T100-registry cwd/commandline sampling (Get-ContinuousCoReviewContainmentSamples riding Get-SpecrewProcessTreeDescendants; Windows exe/command-line-primary, POSIX +cwd); pure checker (Test-ContinuousCoReviewContainmentViolations) reuses the shared physical-path helpers; `containment-violated` loud ORIGIN-SIDE fail at the reviewer's natural end (never mid-flight kill); records carry ONLY bounded/redacted path/process metadata; false-kill guard + paired legit/abuse + orchestrator fail-loud tests (Trace: FR-011, SC-003, NFR-007; owns: process-registry watch path)
