...
- **FR-008**: The downstream-user-facing coordinator HANDOFF prose MUST be audited and rewritten so it names the methodology concept rather than an internal `\bF-\d{3,}\b` / `\bProposal \d{3,}\b` / `\bFeature \d{3,}\b` reference, and the coordinator prompt MUST carry a directive instructing the agent not to emit internal references in handoff-block prose. **Scope (pinned 2026-05-26):** the HANDOFF-block template + coordinator handoff/decision prose emitted to the downstream human (in `scripts/specrew-start.ps1`'s handoff/decision sections and `extensions/specrew-speckit/prompts/coordinator-*.md`). **Explicitly EXCLUDED:** internal project artifacts — `proposals/`, `specs/`, `docs/`, `tests/`, `.squad/decisions/`, `CHANGELOG.md`, and the AI-facing Lifecycle Quick Reference — which legitimately reference feature/proposal numbers. (Note: the brief's named `installed-instructions/` directory does not exist; this FR maps that intent to the actual coordinator handoff-prose surface.) (Proposal 078 Pillar 2b)
- **FR-009**: Governance validation MUST add a regex check that emits a WARN when `\bF-\d{3,}\b` / `\bProposal \d{3,}\b` / `\bFeature \d{3,}\b` patterns appear **within `=== SPECREW HANDOFF ===` … `=== END SPECREW HANDOFF ===` regions** (in committed markdown and the coordinator handoff template). The check MUST NOT scan internal artifact trees outside handoff regions. (Proposal 078 Pillar 5)
...
- **SC-006**: 0 `\bF-\d{3,}\b` / `\bProposal \d{3,}\b` / `\bFeature \d{3,}\b` internal references remain in the in-scope coordinator HANDOFF prose surface (FR-008); the validator regex WARNs on a planted internal reference inside a handoff region and does NOT WARN on the same token in an internal artifact (e.g. a proposal).
