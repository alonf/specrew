...
4. **Module Loader (`Specrew.psm1`) Implementation**: **Resolved 2026-05-16 for Iteration 001** — Option A explicit dot-sourcing. Use `$ScriptRoot = $PSScriptRoot`, dot-source `scripts/internal/dashboard-renderer.ps1` first, then load `scripts/specrew.ps1`, `scripts/specrew-init.ps1`, `scripts/specrew-review.ps1`, `scripts/specrew-start.ps1`, `scripts/specrew-team.ps1`, `scripts/specrew-update.ps1`, and `scripts/specrew-where.ps1` in explicit reviewed order. Use `Join-Path` for every path segment and export the FR-002 public command surface (aliases allowed per implementation convention).
...
| **R2: PowerShell Module Loader Patterns** | Pillar 1 depends on a deterministic `Specrew.psm1` structure | Section in `research.md` documenting loader patterns plus the approved explicit dot-sourcing order, `Join-Path` composition rule, and export expectations | Implementation team (Phase 0) |
