...
- **2026-05-03 Iteration 002 Closeout Review**: When retro evidence already exists, do not trust a green governance-validator run alone. The reliable closeout check is: confirm `review.md` exists, move `plan.md` to `retro`, regenerate `evaluation\report.md` with `process-scorer.ps1 -WriteReport`, and require the scorer to return PASS for both artifact adherence and phase adherence before calling the iteration review/demo complete.
