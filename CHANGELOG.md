...
- **docs(branding)** (this commit): Theme-aware logo assets at `docs/assets/specrew-logo-light.svg` (dark wordmark for light backgrounds) and `docs/assets/specrew-logo-dark.svg` (white wordmark for dark backgrounds). Icon-only variant at `docs/assets/specrew-icon.svg` for favicon / avatar / small-badge use. `README.md`, `docs/getting-started.md`, and `docs/user-guide.md` updated to use GitHub's `#gh-light-mode-only` / `#gh-dark-mode-only` image-fragment switching so the wordmark is legible regardless of the viewer's theme. Tagline updated everywhere to "Governed Agentic SDLC" (was "Governed AI SDLC") — better captures the differentiation that Specrew governs multi-agent software delivery, not just AI-assisted coding. ASCII banner in `specrew init` updated to match. Original PNG asset retained at `docs/assets/specrew-logo.png` for non-theme-aware uses (social cards, etc.).
