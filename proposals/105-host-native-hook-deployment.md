...
- **2026-05-26 PlanningPoC post-compaction discipline drop** — within a SINGLE Squad+Copilot dogfooding session, the same agent instance correctly emitted the three-section handoff at one stop, then dropped the discipline at the IMMEDIATE NEXT stop (right after a compaction event). Pre-compaction discipline ≠ post-compaction discipline, even on the same host with the same coordinator-prompt. This adds a new axis: **prose-prescribed runtime convention degrades within a session whenever context is compressed**. Runtime hooks survive compaction because they live in the host's process configuration, not in the agent's conversation context. Documented in Proposal 120 Sub-trigger 3c.

The escalation is clear: cooperative enforcement is insufficient for hosts that don't honor prose-based STOP signals, AND insufficient even on hosts that DO honor them when compaction degrades the agent's discipline. The incidents are getting worse across two axes — cross-host (1 gate → 4 gates) AND within-host post-compaction. Multi-host expansion accelerates the first axis; long-session execution accelerates the second. **Promote this proposal to high-priority phase-2 and implement before the next dogfooding session.**
