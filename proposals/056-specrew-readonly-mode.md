...
- 2026-08-29: **empirical motivation recorded, and a shipping slice identified.** During F-199 iteration 002 a reviewer session and an implementer session ran concurrently in one governed working directory — the ordinary two-session workflow this proposal anticipates. In a single day the reviewer session, which wrote nothing to the repository, was ordered by Stop hooks to: render a full boundary re-entry packet for another session's crossing **five times** (twice for a crossing minted over a stage with no artifacts); render a material-work context packet after two read-only calls; and **ask the maintainer to spend one of four metered review rounds** on the implementer session's uncommitted files, which also contradicted a standing "one covering round on the tree that ships" ruling. Common root: the machinery keys on repository state and never on whether *this actor* produced it. F-199's T023 ("the owing actor" — ownership recorded on the crossing, demand scoped to it, fail open with disclosure when ownership is indeterminate) is a **slice of this proposal scoped to the boundary-packet path only**; the review-round solicitation path is not covered by it. Recommend 056 absorb T023 as shipped prior art and generalize the ownership question to every hook that spends, commits, or solicits on state alone. Composes with Proposal 216 (Temporal Spec Model): both concern what a non-authoring reader may safely conclude from project state.
