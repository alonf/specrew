...
- **2026-08-29** — Annotated with Proposal 216 (Temporal Spec Model), which supplies the justification this proposal has been carrying implicitly. The three-phase asymmetry — sequential spec PR, then parallel per-developer task branches — is not a convenience or an inherited open-source convention. It follows from what specs and code respectively *are*: **a specification is normative and code is descriptive, so norms must serialize and facts may parallelize.** Two developers cannot both be authoritative about what current intent *is* — that is not a tooling limitation, it is what "normative" means — while they can produce code in parallel and merge it, because facts compose. Recording this because the proposal currently reads as a workflow someone invented, which understates it: the shape is a consequence, and stating the reason makes the boundary between the serialized and parallel phases principled rather than a matter of taste. No scope change.
