...
- 2026-08-29: annotated with Proposal 216 (Temporal Spec Model) cross-reference. **Scope hypothesis to test, not a scope change.** 216 makes the product-level current spec a *derived projection* of sealed iterations rather than an authored document. A derived artifact cannot have a merge conflict — it is recomputed, not merged — so if 216 ships, the spec-content conflict class collapses to concurrent edits of the *active* spec, which Proposal 115's sequential spec-PR phase already serializes. That bears directly on this proposal's open question 2 ("what counts as 'overlapping behavior'?"), which is hard precisely because it requires diffing normative prose between two developers; a projection never performs that diff. **Before this proposal is scheduled, test the hypothesis against its own conflict taxonomy**: enumerate which of the identified conflict types survive a derived current view. A substantial part of the 75 SP may be pricing a problem 216 removes rather than solves. No scope change recorded here until that test is run.
