...
- 2026-05-25: **THIRD empirical instance** — the 4-host smoke test (Antigravity / Codex / Claude / Copilot) on the C++ DirectX dice-app prompt produced TWO independent post-closeout-out-of-lifecycle patterns:
  - **Antigravity (Gemini Flash tier)**: after `approved for feature-closeout` on the dice-app project, 3 code-change commits landed as direct hotfixes (`67521bd`, `91541ab`, `095c0a8`) fixing the dice-penetration physics bug. No iter-002 opened. No spec amendment. Only 1 of 3 fixes got a regression test, and it was the wrong one (face-snap orientation, not penetration). The fix-without-test pattern means the penetration bug class is now unguarded.
  - **Copilot (per-role-routed via Squad, Haiku 4.5 Reviewer)**: after `approved for feature-closeout` on the 001-3d-dice-roller project, the user reported "white screen" → Copilot modified `src/app/win_main.cpp` + `src/render/camera_render_pipeline.cpp` to add D3D11 init + view/projection fixes. User reported "green background only" → Copilot added a rotating-cube debug placeholder to `win_main.cpp` + `CMakeLists.txt`. **NEITHER change opened a new iteration, amended the spec, recorded an FR, or added regression coverage.** Both happened via `general-purpose` Implementer dispatch with no slice-type intake. The methodology layer simply did not exist after feature-closeout.
