...
| [120](120-handoff-block-validator-enforcement.md) | **HIGH PRIORITY (Tier 1)** — Handoff-Block Validator Enforcement + Non-Specrew-Session Bypass Detection — 5-pillar slice: (1) handoff-block presence check + shared `Test-SpecrewHandoffBlockPresent` helper, (2) trigger-bypass diagnosis augmentation, (3) wrong-location warning, (4) state-advance-without-verdict cross-check (2026-05-26 Picard catch `a5866c1`), (5) **review.md-vs-git-ls-tree cross-check (added 2026-05-27 from PlanningPoC iter-004 reviewer-approves-working-tree-only-state — 7 production files cited in review.md never committed, recovered via `29fce04`; MOST CONSEQUENTIAL SHAPE because only one where delivered CODE can permanently vanish)**; empirically motivated by 7 incidents across Shapes 1-5 (multi-host + multi-session + multi-shape pattern); composes with 030 / 067 / 075 / 078 / 105 + companion Reviewer-charter discipline change as separate small-fix slice | phase-2 | 6-10 |
...
| [130](130-specrew-switch-to-host-handover.md) | /specrew-switch-to Slash Command + handover.md Context Bridge (Host-Switch Hand-off Helper) — 3 pillars: Pillar 1 slash command persists state + emits paste-ready exit/relaunch (~3-5 SP, ships independently as foundation); Pillar 2 structured handover.md file with 5 sections (conversation tail / why-switching / open questions / working hypothesis / next-step) + receiving-host bootstrap integration (~4-6 SP); Pillar 3 audit-trail preservation + retro integration (~2-3 SP); empirically motivated by 2026-05-25/26 session's 5+ manual host switches; composes with 064 / 069 / 035 / 078 / 105 / 124; Mode-2 orchestrated switch deferred to future proposal (depends on 024) | phase-2 | 10-15 (3 pillars, Pillar 1 splittable) |
...
| [139](139-multi-agent-subagent-orchestration.md) | **HIGH PRIORITY (Tier 1)** — Multi-Agent Subagent Orchestration (Claude First, Multi-Host Extensible) — 6 pillars: per-host subagent contract (`SubagentInvoker` interface) + Specrew-defined subagent catalog (data-driven YAML at `.specify/subagents/catalog.yml`) + cost-aware routing (absorbs Proposal 068 minimal slice: lean/balanced/premium cost-profile across haiku/sonnet/opus tiers) + validator + boundary-check parallelization (extends Proposal 084 shipped `-Parallel` mechanism) + context-window protection (reduces compaction trigger → reduces Shape 3c discipline drop) + multi-host extension (V1 Claude-first; Codex/Copilot/Antigravity degraded-mode adapters; V2 follow-up when subagent surfaces mature); empirically motivated 2026-05-27 by F-049 token-cost burn + Shape 3c compaction-vulnerability incidents (F-024/F-046/F-048) + this current session's empirical demonstration (Explore + general-purpose subagents used repeatedly during F-049 work); upstream-contribution candidate (Squad's `squad triage --execute` runs single-loop with NO subagent support; Spec-Kit also single-loop; both verified via 2026-05-27 repo audit); user direction explicit 2026-05-27 "speed up and reduce cost"; composes with 024 (foundation host-runtime abstraction) / 068 (absorbs minimal cost-routing slice) / 023 (adjacent specialist activation) / 084 (shipped — extends with subagents) / 086 P1 (shipped — memoization composes) / 105 (shared host-adapter pattern) / 124 (new hosts need adapter) / 137 (3 design flavors parallelize via subagents) / 138 (checklist + analyze are subagent candidates) / 102 (reviewer subagent role partial provision) / 133 (compaction defense-in-depth) / 010 + 134 (multi-dev story compose for "multiple humans + multiple AI agents per human") / 039 (Squad upstream channel) | phase-2 | 15-25 (V1 single-iter; splittable V1a foundation 7-11 + V1b routing 8-14) |
