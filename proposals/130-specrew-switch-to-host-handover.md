...
> **Amendment note (2026-06-08, posture inverted by [Proposal 172](172-hook-driven-session-bootstrap.md)):** the sentence above ("the launcher remains the recommended path") is **superseded** for the bootstrap role. After Feature 171 (Proposal 146) shipped the hook dispatcher, the maintainer inverted the posture: the **SessionStart hook becomes the PRIMARY session bootstrap** (orientation + handover-read + Resume/New/Pick menu), and `specrew start` is **kept for backward-compat + host selection**, not as the orientation path. Proposal 172 owns that inversion plus the F-171 "hooks are non-interactive injectors -> the agent renders the menu" constraint and the B2 escalation. This Pillar 4 (handover format + SessionEnd/SessionStart scripts) remains the implementation 172 composes; 172 does not re-specify it. Recorded explicitly per Proposal 167 cross-proposal amendment discipline.
...
- 2026-06-08: **posture inversion recorded (cross-proposal, [Proposal 172](172-hook-driven-session-bootstrap.md)).** Feature 171 (Proposal 146) shipped the hook dispatcher to v0.33.0-beta1. The maintainer then inverted Pillar 4b's "launcher remains the recommended path": the SessionStart hook becomes the PRIMARY bootstrap (orientation + handover-read + Resume/New/Pick menu); `specrew start` is kept for compat + host selection. Proposal 172 owns the inversion + the hook-injects-directive/agent-renders-menu constraint (F-171/F-165 lesson) + the B2 escalation. Pillar 4's handover format + SessionEnd/SessionStart scripts are unchanged and are the implementation 172 composes. See the amendment note in the Pillar 4b reframe section. Per Proposal 167 discipline.
