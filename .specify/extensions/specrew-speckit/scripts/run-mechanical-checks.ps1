    ...
    foreach ($file in $SourceFiles) {
        ...
        for ($index = 0; $index -lt $lines.Count; $index++) {
            ...
            $null = $findings.Add((New-MechanicalFinding `
                        ...
                        -SourceLine ($index + 1) `
                        -SourceColumn $column `
                        -RequirementRefs @('mechanical quality bar', 'unused-state hygiene') `
    ...
    foreach ($file in $SourceFiles) {
        ...
        for ($index = 0; $index -lt $lines.Count; $index++) {
            ...
            $null = $findings.Add((New-MechanicalFinding `
                        ...
                        -SourceLine ($index + 1) `
                        -SourceColumn $column `
                        -RequirementRefs @('mechanical quality bar', 'deterministic anti-pattern scan') `
                        -RuleDispositions $RuleDispositions))
        }
    }
    ...
    foreach ($file in $TestFiles) {
        ...
        for ($index = 0; $index -lt $lines.Count; $index++) {
            ...
            $null = $findings.Add((New-MechanicalFinding `
                        ...
                        -SourceLine ($index + 1) `
                        -SourceColumn $column `
                        -RequirementRefs @('mechanical quality bar', 'assertion-driven tests') `
                        -RuleDispositions $RuleDispositions))
        }
    }
