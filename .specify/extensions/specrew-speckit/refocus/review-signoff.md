...
4. **Live co-review is mandatory for code-touched review-signoff.** Before accepting `review.md`, invoke `/specrew-review --live --baseline-ref <committed-baseline>` (or `specrew review --live --baseline-ref <committed-baseline>`) with the active feature and iteration context. Treat `.specrew/review/inline/<run-id>/gate-verdict.json` and `.specrew/review/inline/<run-id>/review-run.json` as required evidence. If the live reviewer is unavailable or unsafe, record the infrastructure failure as a blocker or obtain explicit human defer approval; do not silently replace it with hand-authored review prose.
