...
5. **Name the evidence the record rests on.** `review.md` carries `<!-- SPECREW-REVIEW-EVIDENCE: run-... -->`; only the run(s) named there, plus the run named by the derived independent-review block, are checked against the review store. Every other run id in the document is narrative and is never checked - so a retraction may name a failed run freely, and naming a run here that the record does not rely on only invites a refusal. Leave it empty when no campaign run underwrites the record.
6. **Claim ledger honesty.** Every claim in review.md maps to evidence (commit, test run, file inspection); over-strong claims get weakened to what the evidence carries. Check arithmetic claims by re-computing, not re-reading.
7. **Producer-consumer demonstration.** Producer-side changes (metadata, formats, emitted artifacts) need a consumer-side test that proves something real reads them correctly.
8. **Gate-coverage skepticism.** For any gate/validator the feature touched: does it cover what its spec CLAIMS it covers — both directions, all paths? Directional blind spots ship real bugs.
9. **Whole-file re-reads at closure.** Re-read changed artifacts end-to-end; grep for stale-phrase CLASSES (old names, old counts, old statuses), not just phrases that failed before.
10. **The verdict names its exact boundary.** "APPROVE for review-signoff" — never an ambiguous "approve" that could be read as advancing further than reviewed.
