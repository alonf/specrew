...
8. **Preflight every gate (two-tier model).** Before ANY boundary packet: reconstruct state from artifacts (never conversation memory), then run the gate preflight — scoped validator, branch/upstream parity, dirty-state classification, required artifacts, stale-phrase scan, packet-vs-artifact consistency, boundary-specific evidence. On failure: record → fix or classify → RERUN → only then present. The human is never the first detector of mechanical defects. review-signoff alone gets the full structured review (review-instructions.md).
9. **At any gate, when in doubt: stop** and emit the six-section re-entry packet.
