...
9. **Render the full six-section re-entry packet when the stop requires the human to RE-ORIENT — but reply as a normal conversation on a quick discussion turn.** Render the packet (What I Just Did / Why I Stopped / What Needs Your Review / What Happens Next / Discussion Prompts / What I Need From You) when ANY of these holds: (a) you are at a boundary GATE; (b) you did MATERIAL WORK this turn — changed code, ran tests, wrote or committed files; (c) the turn ran LONG enough that the human likely stepped away while you worked and is re-engaging when they return. OMIT the packet and answer conversationally when the human is present and the turn was a quick discussion / question / short lookup with NO code change, NO tests, and NO gate. A long message is NOT, by itself, a reason for the packet. The test: reading only your message, would the human need to re-focus on where the work stands? If yes → render the packet; if you just discussed or did something fast while they were watching → give a direct answer. Never collapse the packet into a picker/menu, which hides it behind its short fields. At a BOUNDARY stop, ALSO emit the verdict marker `<!-- SPECREW-VERDICT-BOUNDARY: <from> -> <to> -->` as the LAST line so the human's verdict is captured into the gate's authorization. Render and collect the verdict through your host's approved interaction path for a verdict stop: use your host's dedicated boundary-stop surface if it has one, otherwise render the packet directly — full sectioned markdown with bare `file:///` URLs and — at a BOUNDARY stop — a numbered verdict block, never a compressed "1. Why:… 2. What:…" list. (A within-phase checkpoint owes the packet but NOT verdict-style numbered options, just as it owes no verdict marker.) The design workshop's per-lens questions and clarify-stage ambiguity questions are NOT decision-yield stops — they keep their normal picker/question path.
