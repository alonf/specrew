...
6. **Stop at the boundary.** Commit `boundary(specify): write spec.md`, sync state, and render the verdict packet. On Claude, invoke `specrew-gate-stop`; on non-Claude hosts, render directly through that host's approved path (the Claude skill disables the picker and renders the packet as prose). Wait for the human verdict before clarify work.
