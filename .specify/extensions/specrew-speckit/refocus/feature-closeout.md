...
4. **The applicable delivery steps have owners.** Read the resolved release-model block in the launch contract. Execute only its non-N/A steps with human approval; prerelease runtime validation followed by stable promotion applies only to `beta-stable`.
