...
function Get-SpecrewHostSupportTierRows {
    ...
    $rows = @(
        ...
        [pscustomobject][ordered]@{ host = 'copilot'; surface = 'cli'; tier = 'verified'
            rationale = 'CLI hook contract verified: user-level sessionStart/agentStop fire in -p AND interactive; agentStop decision:block gate + fail-open confirmed (runner-observed probe).'
            provenance = 'RUNNER-OBSERVED by the REPRODUCIBLE, DIGEST-BOUND probe tests/host-probes/copilot-hook-firing-probe.ps1 (recorded at the reviewed digest; 2026-07-15, Copilot CLI 1.0.70): USER-level hooks FIRE in headless copilot -p from an UNTRUSTED cwd (governance rides the user hook; not trust-gated), while REPO-level hooks from an untrusted folder do NOT fire (the trustedFolders gate). User-hook firing in the INTERACTIVE surface plus the agentStop decision:block gate + fail-open behaviors are documented in the recorded characterization; the PTY-less probe reproduces the -p surface, and the interactive surface stays characterization-documented (not probe-reproduced).'
