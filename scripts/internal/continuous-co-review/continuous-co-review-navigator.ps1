...
function Test-ContinuousCoReviewNavigatorSupervisorAlive {
    # Is the registry's supervisor pid still a live process? Absent/zero pid -> not alive.
    param([AllowNull()]$Registry)
    if ($null -eq $Registry -or -not ($Registry.PSObject.Properties.Name -contains 'supervisor_pid')) { return $false }
    $supPid = $Registry.supervisor_pid
    if (-not $supPid) { return $false }
    try { $null = Get-Process -Id ([int]$supPid) -ErrorAction Stop; return $true }
    catch { return $false }
}
...
function Clear-ContinuousCoReviewNavigatorEntry {
    ...
    # record the gate enforces lives separately in .specrew/review/runs/ (promotion is not this cut).
...
function Invoke-ContinuousCoReviewNavigatorReap {
    ...
    #   - running + past-deadline + supervisor ALIVE: the supervisor overran its own kill loop (or is
    #     wedged) -> Stop-SpecrewIsolatedTask (kill + worktree cleanup + mark reaped).
    #   - running + supervisor GONE + no terminal status: a DEAD launcher/supervisor orphan ->
    #     Stop-SpecrewIsolatedTask marks it crashed + cleans the orphaned worktree (the backstop the
    #     launcher's own finally-dispose cannot cover when the supervisor itself was killed).
    #   - running + supervisor alive + within deadline: leave it (still working).
    # Returns @{ stop_block; inject_notes[]; reaped_run_ids[] }. stop_block is the FIRST blocking
    # verdict's directive (one STOP-BLOCK per stop). CrossSession skips verdict surfacing (a prior
    # session's verdict is stale for THIS turn) and only kills+cleans orphans.
    param(
        [Parameter(Mandatory)][string]$RepoRoot,
        [switch]$CrossSession,
        [datetime]$Now = [datetime]::UtcNow
    )
    $result = [pscustomobject]@{ stop_block = $null; inject_notes = (New-Object System.Collections.Generic.List[string]); reaped_run_ids = (New-Object System.Collections.Generic.List[string]) }
    ...
    foreach ($entry in (Get-ContinuousCoReviewNavigatorPendingEntries -RepoRoot $RepoRoot)) {
        ...
        $alive = Test-ContinuousCoReviewNavigatorSupervisorAlive -Registry $reg
        ...
        $shouldStop = $false
        $reason = 'reaped'
        if ($pastDeadline -and $alive) {
            # Overran its own kill loop / wedged: kill + clean.
            $shouldStop = $true; $reason = 'reaped'
        }
        elseif (-not $alive) {
            # Supervisor gone with no terminal status: a DEAD-launcher orphan (worktree may have leaked).
        ...
        # else: alive + within deadline (current session) -> leave it running.
    }
    return $result
}
...
function Invoke-ContinuousCoReviewNavigator {
    ...
    # 1) REAP first (always). SessionStart -> cross-session sweep (orphan kill/clean only).
    $reap = Invoke-ContinuousCoReviewNavigatorReap -RepoRoot $RepoRoot -CrossSession:$SessionStart -Now $Now
    ...
    # terminal + orphaned entries; this handles a still-alive-within-deadline prior).
    foreach ($entry in (Get-ContinuousCoReviewNavigatorPendingEntries -RepoRoot $RepoRoot)) {
        $reg = $entry.registry
        $status = if ($null -ne $reg -and ($reg.PSObject.Properties.Name -contains 'status')) { [string]$reg.status } else { '' }
    ...
    if (-not (Get-Command -Name 'Start-SpecrewIsolatedTask' -ErrorAction SilentlyContinue)) {
        ...
        return $decision   # degraded: cannot fire; fail-open no-op (the loader WARNed once).
    }
