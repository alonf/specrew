...
function Get-SpecrewDashboardSnapshot {
    ...
    if (-not $roadmapDefinition.exists) {
        $warnings.Add('No .specrew/roadmap.yml file found. Add one to enable roadmap progress and remaining-effort projection.') | Out-Null
    }
...
function ConvertTo-SpecrewDashboardLines {
    ...
    if ($Snapshot.roadmap_progress.Count -eq 0) {
        $lines.Add('Roadmap unavailable; add .specrew/roadmap.yml to enable this section.') | Out-Null
    }
    else {
