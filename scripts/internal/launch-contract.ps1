...
46A. **End every turn by running the turn-end script.** ``pwsh -File .specify/extensions/specrew-speckit/scripts/declare-turn-end.ps1 -Kind <boundary|in-flight|conversational> -Summary '<what this turn did>' -Token <the token from the latest [specrew-turn] line>`` as the last action of the turn, then output whatever it returns, verbatim - it may return nothing, and nothing is a complete answer. The hook hands you the token in one line beginning ``[specrew-turn]`` at the start of each turn; pass the most recent one, because it is how the declaration is placed under this session when more than one is working in the project. Use ``-Owed '<artifact>'`` when the stage owes something it has not produced, and ``-Pending '<the work>'`` with ``-Kind in-flight``. Do not compose the packet yourself: the script renders it from the artifacts, and the boundary, approval phrase and verdict marker come from ``.specrew/runtime/pending-verdict-stop.md`` rather than from the phase you intend to enter next. The ``specrew-turn-end`` skill carries the detail.
