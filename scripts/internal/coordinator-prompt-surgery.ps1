...
"@
    ...
    if (-not [string]::IsNullOrWhiteSpace($primitive)) {
        if ([string]::IsNullOrWhiteSpace($guidance)) {
            $guidance = "Use the $displayName structured user-input/menu primitive for clarify questions and design discussion when it is available. NOT for a boundary verdict: only a typed phrase is captured there, so a selection primitive at the verdict offers a control that cannot authorize."
        }
...
Render the Rule 46 six-section re-entry packet as PROSE in your message FIRST, then stop for the human's TYPED reply. Do NOT call the structured primitive for a boundary verdict on any host (maintainer ruling 2026-08-12): only a typed phrase is captured, so a picker at a boundary verdict offers a control that cannot authorize -- measured, a selection was accepted by the interface, nothing was captured, and the agent then invoked the authorization writer directly. NO SELECTION AFFORDANCE AT A BOUNDARY VERDICT. This binds the verdict step only; pickers in design discussion and clarify questions, where nothing is recorded and no boundary advances, are unaffected. The primitive's fields are short labels by design (a ~12-character header, 1-to-5-word option labels) and CANNOT carry the packet -- the What I Just Did / Why I Stopped / What Needs Your Review / What Happens Next / Discussion Prompts content, with its file:/// links, lives in your prose ABOVE the call. A verdict menu raised without the full packet rendered above it (a bare "What's your verdict?" that skipped the packet) is a Rule 46 violation, not a valid stop -- the menu is only the short picker that follows the rendered packet. Render the four responses as literally sendable lines: "approved for <to>", "approved for <to> - <your instructions>", "changes needed: <what to change>", "discuss prompt 1". All four kinds are kept -- approve-with-instructions is how a human approves without rubber-stamping. Initial feature intake may remain free-form. Clarify questions should use structured choices when the expected answer set is known; otherwise ask a concise free-form question.
"@
    }

    return @"
Host-rendered interaction guidance for ${displayName}:
No structured question/menu primitive is declared for this host package. At every approval boundary render the four responses as lines the human can literally send - "approved for <to>", "approved for <to> - <your instructions>", "changes needed: <what to change>", "discuss prompt 1" - and stop for the typed reply. Initial feature intake may remain free-form. Clarify questions should use structured choices when the expected answer set is known and a structured primitive is available; otherwise ask a concise free-form question.
"@
}
