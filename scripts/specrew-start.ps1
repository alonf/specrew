...
9a. **Lens intake is a per-lens facilitated design workshop** (Amendment A4) — part of specify, completed before the specify boundary is synced so the accepted spec is lens-informed (FR-027); it is NOT a binary applicability questionnaire. For a substantive feature in a project that ships the design-lens catalog: (a) **Infer applicability, then confirm** — propose which lenses apply WITH your reasoning (a screen-capture app plainly has UI; a payments feature plainly has security/PII) and ask the human only to confirm or adjust. Do NOT make the human answer obvious yes/no applicability, and do NOT silently auto-resolve a material area. (b) **Per-lens discussion** — for each applicable lens act as workshop coordinator: raise that lens's design questions (its decision points — e.g. UI -> theme, controls, layout, contrast, DPI, smoothness, technology; data -> storage model, consistency, retention), offer options where useful, capture the human's decision and explicit agreement, and ITERATE until the human says "move on" before going to the next lens. (c) **Adapt depth** to the user-profile expertise dials (concise expert-level questions where a dial is high; explain and recommend a default where low). (d) **Right-size** to the feature, the inferred applicability, and expertise — not a fixed nine-lens marathon; MCQ may be used WITHIN the discussion, but the intake is a guided conversation until the human is done, never a one-shot multiple-choice prompt. Record the feature-level specs/<feature>/lens-applicability.json with ``workshop_intake: true``, the ``selected`` lenses, and a per-lens ``workshop`` record for each selected lens capturing its **agenda** (the questions raised), the **decision/agreement** summary, the **depth** used, and a **moved_on** marker (the SC-021 floor). Then amend spec.md plus the requirements checklist with the lens-informed requirements before sync-specify. If you can run the workshop before invoking speckit.specify, do so; otherwise let Spec Kit scaffold the initial spec draft first, then run the workshop, record the artifact, and amend the spec and checklist before sync-specify. Do not treat "after specify is synced, before clarify" as acceptable. The specify-boundary gate refuses sync-specify until the feature-level lens-applicability.json exists, and the SC-021 floor (the pre-plan design-analysis gate) refuses plan until each selected lens carries a non-placeholder workshop record — but those are PRESENCE checks only; the workshop's QUALITY is yours to run well (it is validated by the human experience, not the gate). The selected lenses' decisions then inform clarify, the design analysis, and the plan (FR-009). (e) **Frame the phases (Amendment A6 / FR-034)** — at the workshop outset, tell the human what this step is and is NOT: the workshop *gathers* the inputs, constraints, and decisions; the system STRUCTURE (the components, their responsibilities, and the flows) is designed WITH the human at the later design-analysis step (Rule 9c), not decided here — so the human knows the collaboration is coming and what each phase settles.
9b. **Workshop visuals (Amendment A5).** During the workshop and design discussions you MAY make a lens's discussion concrete with its **native diagram** from the per-lens diagram vocabulary (``extensions/specrew-speckit/knowledge/design-lenses/diagram-vocabulary.json``): security -> trust-boundary/attack-surface; data -> ERD or NoSQL document relations (references by JSON-path of identities); architecture -> component/service/flow; devops -> deployment topology; ui-ux -> layout/flow/state; cross-cutting -> sequence flows and comparison tables. YOU author the diagram content; surface it **in-band — never written to disk only (Amendment A6 / FR-037)**. **On a terminal/console host (Claude Code, Codex, Copilot CLI) a fenced ``mermaid`` block is SOURCE TEXT, not a rendered picture — only console ASCII actually renders as a diagram the human can see.** So the inline diagram you show the human MUST be **console ASCII art rendered directly in your message** (boxes, arrows, labels) — reach for ASCII FIRST and by default. Use **mermaid / svg / html** only as a *richer, additional* artifact written to a file, and when you do you MUST emit its clickable ``file:///`` link in the SAME message (a mermaid block, or a file the human cannot see, does NOT count as surfaced). NEVER let the human reach a decision while the diagram lives only in a file or only as mermaid source. A per-lens diagram is **expected, not optional**, for the structural lenses (architecture-core, data-storage, security-compliance, integration-api, devops-operations) and for any UI-bearing feature (ui-ux — a layout/flow/state sketch); keepers persist as **inline mermaid** in the design doc. Intake is **bidirectional**: per applicable lens, offer to plot the lens's diagram AND ask whether the human has an existing diagram, document, Figma export, or whiteboard photo — reference a provided artifact via a ``file:///`` link and optionally transcribe a drawing to mermaid so it persists (accept a path/image; no live API). Temp visuals are ephemeral (gitignored, cleaned at iteration close); keepers are mermaid-inline (versioned text). The diagram CONTENT is yours to author well; a deterministic helper just writes the file and emits the link.
9c. **Design-analysis is a co-design session, not a unilateral deliverable (Amendment A6).** At the design-analysis stop for a substantive feature, do NOT hand the human three finished architectures to pick from. Co-design WITH the human first: (a) **Co-decide the design method / decomposition style (FR-035)** — raise it as an expertise-adapted discussion (e.g. DDD bounded-contexts, IDesign volatility-based, modular monolith, microservices, layered), explain the candidates and trade-offs at a depth matched to the architecture dial, recommend a default where the dial is low, and record the chosen method as a binding constraint for the analysis and the plan; it is NOT a bare multiple-choice prompt and you MUST NOT silently assume it. (b) **Co-build the component map (FR-036)** — propose a candidate component/service breakdown and present **every component BY NAME with its one-line responsibility** explicitly in the conversation (e.g. ``CatalogManager — owns plan + exercise CRUD and search``), NEVER a bare count such as '4 managers, 3 engines' (a count is not something the human can react to); render it as an **in-band console-ASCII diagram** (Rule 9b), and invite the human to rename, split, merge, or reassign; walk at least one key **flow** (user actions + system actions) through those components together; ITERATE until the human agrees the decomposition, responsibilities, and flows are right. (c) **Only then present the remaining trade-off options** (e.g. transport, store technology, deployment granularity) for the human's verdict — the options are the *leftover* consequential choices, not the whole design handed down. (d) **Adapt to expertise** — where the architecture dial is low you MAY drive the decomposition and explain more, but you MUST still confirm the responsibilities and flows with the human rather than authoring them silently. (e) **Record the co-design (SC-025)** — write the agreed component-to-responsibility map and at least one agreed flow into a ``## Co-Design Record`` section of ``design-analysis.md`` with an explicit human-agreed marker, and set ``co_design: true`` in the iteration's ``lens-applicability.json`` so the deterministic co-design-record floor applies. **When ui-ux is in scope, ALSO capture the agreed UI/screen layout** in that record (a ``UI layout (agreed):`` entry — the console-ASCII sketch the human approved, or a precise description), because an agreed layout that lives only in the chat scrollback is lost; the floor requires this whenever ``ui-ux`` is a selected lens. The co-design CONDUCT is yours to run well; the floor only checks the record is present — the QUALITY is validated by the human experience and the runtime dogfood (SC-024), not the gate.
