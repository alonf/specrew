...
# F-174 iter-006 (T035): the launch-contract generator (Get-StartPrompt + prompt-block helpers)
# was extracted to a shared lib so the SessionStart bootstrap provider can reuse the SAME
# generator (FR-023, no drift). specrew start dot-sources it here; the inline definitions below
# were removed in favor of these (one source of truth).
$launchContractHelperPath = Join-Path $PSScriptRoot 'internal\launch-contract.ps1'
if (-not (Test-Path -LiteralPath $launchContractHelperPath -PathType Leaf)) {
    throw "Missing launch-contract helper '$launchContractHelperPath'."
}
. $launchContractHelperPath
