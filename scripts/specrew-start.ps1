...
9a. Lens-applicability intake is part of specify and MUST complete before the specify boundary is synced, so the accepted spec is lens-informed (FR-027). For a substantive feature in a project that ships the design-lens catalog: pose the applicability questionnaire (user-facing UI? auth/secrets/PII/compliance? persistent data? external service/API? deployment/CI/release? performance/scale or resilience?) to the human as actual questions and never silently auto-resolve a material area; adapt question depth to the user-profile expertise dials (concise expert-level questions where a dial is high, explain and recommend a default where low); surface the selected lenses for confirmation; record the feature-level specs/<feature>/lens-applicability.json; and amend spec.md plus the requirements checklist with the lens-informed requirements before sync-specify. If you can ask the lens questions before invoking speckit.specify, do so; otherwise let Spec Kit scaffold the initial spec draft first, then run the intake, record the artifact, and amend the spec and checklist before sync-specify. Do not treat "after specify is synced, before clarify" as acceptable. The specify-boundary gate enforces this: sync-specify is refused until the feature-level lens-applicability.json exists. The selected lenses then inform clarify, the design analysis, and the plan.
