...
| design-analysis stop | For substantive features, compares 2-3 design alternatives, records the Crew recommendation, and requires the human verdict shape ``approved for plan with Option <X>`` before planning proceeds. | ``specs/<feature>/iterations/<NNN>/design-analysis.md`` | active new substantive ``plan`` sync hard-blocks when artifact, recommendation, or Human Decision evidence is missing |
