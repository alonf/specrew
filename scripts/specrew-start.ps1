...
| design-analysis stop | For substantive features, compares 2-3 design alternatives, records the Crew recommendation, and requires the human verdict shape ``approved for plan with Option <X>`` before planning proceeds. The Crew scaffolds ``design-analysis.md`` from the template and MUST NOT author substantive ``plan.md`` until the pre-plan validator passes (artifact valid + recommendation populated + Human Decision recorded). | ``specs/<feature>/iterations/<NNN>/design-analysis.md`` | active new substantive ``plan`` sync hard-blocks when evidence is missing, and the coordinator pre-plan check blocks authoring ``plan.md`` before a valid artifact and human decision exist |
